"""Match AZ dataset against AM Best AZ PPA + Homeowners Multi-Peril reports.

AM Best AZ PDF (2026-05-26 export) contains BOTH PPA and Homeowners Multi-Peril
...
AMBEST_CSV = Path("tools/ambest_az_data.csv")
...
# ---- Load AM Best AZ rows and dedupe ----------------------------------------
seen: set[tuple] = set()
...
print(f"AM Best AZ: {len(raw_rows):,} raw rows -> {len(ambest_unique):,} unique after dedup")
...
print(f"AM Best AZ in-scope PPA: {len(ambest_ppa)}")
print(f"AM Best AZ in-scope HO:  {len(ambest_ho)}")
...
az_rows = [d for d in data if d["state"] == "AZ"]
...
az_ppa = [d for d in az_rows if _is_ppa(d)]
az_ho  = [d for d in az_rows if _is_ho(d)]
az_other = [d for d in az_rows if not _is_ppa(d) and not _is_ho(d)]
print(f"\nOur AZ total: {len(az_rows)} | PPA bucket: {len(az_ppa)} | HO bucket: {len(az_ho)} | other: {len(az_other)}")
...
def cross_check(ambest_rows: list[dict], our_rows: list[dict], our_other: list[dict], label: str):
    ...
    print()
    print("=" * 70)
    print(f"AM Best AZ {label} cross-check")
    print("=" * 70)
    ...
    emit_from_tiers("AZ", label, our_rows, matched_tier1, matched_tier2, reclass, truly_missing,
...
cross_check(ambest_ppa, az_ppa, az_ho + az_other, "PPA")
cross_check(ambest_ho,  az_ho,  az_ppa + az_other, "HO")
...
report_extras(ambest_ppa, az_ppa, "PPA")
report_extras(ambest_ho,  az_ho,  "HO")
