"""Full AZ enrichment run: all target-company filings in Arizona.

Mirrors `run_ut_full.py` with AZ-specific paths. Loads the search-only workbook
(`output/az_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/az_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "az_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "az_final.xlsx"
DEFAULT_STATE = "AZ"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"AZ FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
