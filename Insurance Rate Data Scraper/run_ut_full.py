"""Full UT enrichment run: all target-company filings in Utah.

Mirrors `run_or_full.py` with UT-specific paths. Loads the search-only workbook
(`output/ut_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/ut_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "ut_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "ut_final.xlsx"
DEFAULT_STATE = "UT"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"UT FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
