"""Union HI search-workbook generations into hi_all_companies_search.xlsx.

Mirrors merge_vt_search.py (union + dedupe by filing_id, dated-rows-win), with
an EXPLICIT ordered input list so the in-place rewrite of the main universe is
unambiguous:

  1. hi_all_companies_search.xlsx        (the cold-sweep base — 19 clean terms;
                                          read first, wins dups)
  2. hi_universe_completion_retry.xlsx   (the HI completion sweep — the 6 walled
                                          terms: Farmers family + USAA subs)

Both generations are --no-dates (undated), so on a duplicate filing_id the
first-read (base) copy is kept; new filing_ids from the retry are added.
"""
...
MAIN = OUTPUT_DIR / "hi_all_companies_search.xlsx"
GENS = [
    MAIN,
    OUTPUT_DIR / "hi_universe_completion_retry.xlsx",
    OUTPUT_DIR / "hi_travelers_decap.xlsx",  # 100-cap de-cap: 149 distinct, 0 in-target (confirms genuine absence)
]
