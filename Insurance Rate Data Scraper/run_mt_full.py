"""Full MT enrichment run: all target-company filings in MT.

Mirrors `run_ut_full.py` with AZ-specific paths. Loads the search-only workbook
(`output/mt_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/mt_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "mt_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "mt_final.xlsx"
DEFAULT_STATE = "MT"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"MT FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
