"""Match NV dataset against AM Best NV PPA + Homeowners Multi-Peril reports.

AM Best NV PDF (2026-05-26 export) contains BOTH PPA and Homeowners Multi-Peril
...
AMBEST_CSV = Path("tools/ambest_nv_data.csv")
...
print(f"AM Best NV: {len(raw_rows):,} raw rows -> {len(ambest_unique):,} unique after dedup")
...
print(f"AM Best NV in-scope PPA: {len(ambest_ppa)}")
print(f"AM Best NV in-scope HO:  {len(ambest_ho)}")
...
nv_rows = [d for d in data if d["state"] == "NV"]
...
print(f"\nOur AZ total: {len(nv_rows)} | PPA bucket: {len(nv_ppa)} | HO bucket: {len(nv_ho)} | other: {len(nv_other)}")
...
def cross_check(ambest_rows: list[dict], our_rows: list[dict], our_other: list[dict], label: str):
    ...
    print()
    print("=" * 70)
    print(f"AM Best NV {label} cross-check")
    print("=" * 70)
