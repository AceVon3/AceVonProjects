"""Match NM dataset against AM Best NM PPA + Homeowners Multi-Peril reports.

AM Best NM PDF (2026-06-08 export) contains BOTH PPA and Homeowners Multi-Peril
filings (same single-PDF both-line layout as AZ/MT/NV). The PPA bucket folds
sub-types 19.0000/19.0001/19.0002/19.0004 (matching UT/OR). The HO bucket folds
sub-types 04.0000-04.0005.

Reads our NM rows from output/nm_final_rates.xlsx (sheet "rates") so the
cross-check runs at ingestion BEFORE NM is folded into all_states (per
CROSS_CHECK_STANDARD.md). Window-filtered to DATE_TO=2026-04-17 so AM Best
entries past our cutoff (the ~7 weeks to 2026-06-08) don't generate false
...
AMBEST_CSV = Path("tools/ambest_nm_data.csv")
DATASET_XLSX = Path("output/nm_final_rates.xlsx")
DATASET_SHEET = "rates"
...
# ---- Load AM Best NV rows and dedupe ----------------------------------------
seen: set[tuple] = set()
...
print(f"AM Best NM: {len(raw_rows):,} raw rows -> {len(ambest_unique):,} unique after dedup")
...
print(f"AM Best NM in-scope PPA: {len(ambest_ppa)}")
print(f"AM Best NM in-scope HO:  {len(ambest_ho)}")
...
nv_rows = [d for d in data if d["state"] == "NM"]
...
nv_ppa = [d for d in nv_rows if _is_ppa(d)]
nv_ho  = [d for d in nv_rows if _is_ho(d)]
nv_other = [d for d in nv_rows if not _is_ppa(d) and not _is_ho(d)]
print(f"\nOur NM total: {len(nv_rows)} | PPA bucket: {len(nv_ppa)} | HO bucket: {len(nv_ho)} | other: {len(nv_other)}")
...
def cross_check(ambest_rows: list[dict], our_rows: list[dict], our_other: list[dict], label: str):
    ...
    print()
    print("=" * 70)
    print(f"AM Best NM {label} cross-check")
    print("=" * 70)
    ...
    if truly_missing:
        print(f"\n--- Still missing from our NM {label} dataset ({len(truly_missing)}) ---")
    ...
    emit_from_tiers("NM", label, our_rows, matched_tier1, matched_tier2, reclass, truly_missing,
...
cross_check(ambest_ppa, nv_ppa, nv_ho + nv_other, "PPA")
cross_check(ambest_ho,  nv_ho,  nv_ppa + nv_other, "HO")
...
def report_extras(ambest_rows, our_rows, label):
    ...
    for d in our_rows:
        ...
        if k not in ambest_keys:
            extras.append(d)
    print(f"\n--- Our NM {label} not in AM Best ({len(extras)}) ---")
    for d in extras:
...
report_extras(ambest_ppa, nv_ppa, "PPA")
report_extras(ambest_ho,  nv_ho,  "HO")
