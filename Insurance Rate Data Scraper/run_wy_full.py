"""Full WY enrichment run: all target-company filings in WY.

Mirrors `run_ut_full.py` with AZ-specific paths. Loads the search-only workbook
(`output/wy_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/wy_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "wy_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "wy_final.xlsx"
DEFAULT_STATE = "WY"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"WY FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
