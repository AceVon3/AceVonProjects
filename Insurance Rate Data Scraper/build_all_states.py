...
README_LINES: list[tuple[str, str]] = [
    ...
    ("external_validation", "External AM Best validation state (2026-06-04 tier-honesty pass). validated = documented per-row all-field AM Best Disposition Page Data match, value unchanged (only the SFMA-134676753 anchor, 2 rows). unvalidatable = back-extension rows inside the AM Best window that postdate the cross-checks (the 19 window rows; no AM Best export covers them). unvalidated = everything else (eff-2024, and the original rows whose 'validated' claim is unsubstantiated — the per-state cross-checks were aggregate coverage analyses, not per-row validation; CO had none)"),
    ("validation_tier", "Re-derived 1:1 from external_validation, kept for app compatibility: validated->ambest_validated, unvalidatable->ambest_window_unmatched, unvalidated->pipeline_only. NOTE: this column no longer asserts provenance — use `source` for original-vs-extension. See TIER_RELABEL.md"),
    ("", ""),
    ("LIMITATIONS", ""),
