"""Full GA enrichment run: all target-company filings in GA.

Mirrors `run_nm_full.py` paths (inherits download_pdfs=False — metadata-only
enrichment, no attachment downloads; rate data is re-derived from
filing_summary.pdf by run_final_rates). Loads the search-only workbook
(`output/ga_all_companies_search.xlsx`), hydrates Filing objects, then runs
detail-page enrichment group-by-group.

Final output: `output/ga_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "ga_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "ga_final.xlsx"
DEFAULT_STATE = "GA"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"GA FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
...
def main() -> int:
    # RESUME: if a partial ga_final.xlsx exists (a prior run that crashed/stopped
    # part-way), load it instead of the raw search workbook so already-enriched
    # filings are preserved and skipped below. type_of_insurance (parent TOI) is
    # populated only by detail-page enrichment (0% in the search workbook), so it
    # is the resume signal. Fresh runs (no FINAL_XLSX) load the search workbook
    # and enrich everything as normal.
    src = FINAL_XLSX if FINAL_XLSX.exists() else SEARCH_XLSX
    print(f"[load] {src}", flush=True)
    filings = _load_filings(src)
    already_enriched = sum(1 for f in filings if f.type_of_insurance)
    print(f"[load] {len(filings)} filings hydrated ({already_enriched} already enriched -> will skip)", flush=True)
    ...
    with sync_playwright() as pw:
        ...
        try:
            for gi, (state, company) in enumerate(group_order, 1):
                ...
                todo = [f for f in group if not f.type_of_insurance]
                print(
                    f"\n[group {gi}/{len(group_order)}] {state} / {company}: "
                    f"{len(group)} filings ({len(group)-len(todo)} already enriched, {len(todo)} to do)",
                    flush=True,
                )
                if not todo:
                    print("  all enriched, skipping group (no search)", flush=True)
                    continue
                ...
                try:
                    ...
                    for i, f in enumerate(todo, 1):
                        # download_pdfs=False: enrichment scrapes detail-page
                        # metadata only and SKIPS the attachment download/parse
                        # (Rate Manuals / Factor Pages, up to 10/filing). Those
                        # attachments feed NOTHING in the canonical deliverable —
                        # run_final_rates re-derives every rate value, disposition,
                        # and effective date from filing_summary.pdf, which it
                        # downloads separately. Dropping the attachment storm
                        # removes the 60s-timeout-prone downloads that dominated
                        # the NM enrichment runtime (audit decision A, 2026-06-08).
                        enrich_filing(page, f, download_pdfs=False)
                        if i % 10 == 0 or i == len(todo):
                            elapsed = (time.time() - started) / 60
                            print(
                                f"    [checkpoint] {i}/{len(todo)} enriched "
