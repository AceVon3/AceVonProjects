"""Full NV enrichment run: all target-company filings in NV.

Mirrors `run_ut_full.py` with AZ-specific paths. Loads the search-only workbook
(`output/nv_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/nv_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "nv_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "nv_final.xlsx"
DEFAULT_STATE = "NV"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"NV FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
