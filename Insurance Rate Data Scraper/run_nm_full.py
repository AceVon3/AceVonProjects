"""Full NM enrichment run: all target-company filings in NM.

Mirrors `run_nv_full.py` paths. Loads the search-only workbook
(`output/nm_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/nm_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "nm_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "nm_final.xlsx"
DEFAULT_STATE = "NM"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"NM FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
