"""HI scrape vs AM Best HI cross-check (offline, no SERFF, no app changes).

Adapts compare_vt_ambest.py to HI. Same 3-part structure:

  (a) STANDARD  — of the AM Best HI in-scope rows (our 13 brands, Rate, 2025+
  ...
  (b) INTERIM QUALITY — for (entity, eff) pairs present in BOTH, do the rate
      impacts AGREE? Disagreements are the interim-vs-real divergences. The 7th
      interim->real data point (VA 97.7% / OH 100% / IL 99.4% / WV clean /
      NH 100% / VT 100%).
...
HI is small-N. HO ESPECIALLY small (~7 rows -- read the % WITH the counts; PPA at
~44 is the robust signal, the VA small-N HO precedent). Gap class closed this
state, and American Family is pre-resolved: its in-target HI filings are ALL
excluded subs (Munich Re American Family Home + American Family Connect), so
AM Best HI's 0 AmFam is CORRECT -- expect 0 genuine AmFam soft-miss, and the
cleanest soft-miss profile (routine recency / immaterial AM-Best-only rows).
...
AMBEST_CSV = Path("tools/ambest_hi_data.csv")
DATASET_XLSX = Path("output/hi_final_rates.xlsx")
DATASET_SHEET = "rates"

# 13-brand keyword/exclusion sets, inlined verbatim from compare_vt_ambest.py.
...
print(f"AM Best HI: {len(raw_rows):,} raw rows -> {len(ambest_unique):,} unique after dedup")
...
print(f"AM Best HI in-scope (our brands, Rate, {DATE_FROM:%Y-%m-%d}..{DATE_TO:%Y-%m-%d}): "
...
hi_rows = [d for d in data if d["state"] == "HI"]
...
hi_ppa = [d for d in hi_rows if _is_ppa(d)]
hi_ho  = [d for d in hi_rows if _is_ho(d)]
hi_other = [d for d in hi_rows if not _is_ppa(d) and not _is_ho(d)]
pre2025 = [d for d in hi_rows if (_our_eff_dt(d) and _our_eff_dt(d) < DATE_FROM)]
print(f"\nOur HI total: {len(hi_rows)} | PPA: {len(hi_ppa)} | HO: {len(hi_ho)} | other: {len(hi_other)}"
...
def cross_check(ambest_rows, our_rows, our_other, label):
    ...
    print(f"\n{'='*68}\n(a) STANDARD cross-check — AM Best HI {label} -> our scrape\n{'='*68}")
...
t1p, t2p, rcp, missp = cross_check(ambest_ppa, hi_ppa, hi_ho + hi_other, "PPA")
t1h, t2h, rch, missh = cross_check(ambest_ho,  hi_ho,  hi_ppa + hi_other, "HO")
...
print(f"  Our scraped rows NOT in AM Best (extras): {len(extras)} of {len(hi_rows)}")
...
print(f"    {Counter((d.get('sub_type_of_insurance') or '?') for d in hi_rows).most_common()}")
...
blank_mm = [d for d in hi_rows if _blank_maxmin(d)]
print(f"\n[parser-bug watch] our HI rows with blank max OR min %: {len(blank_mm)} "
