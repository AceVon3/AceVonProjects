"""Parse AM Best NM rate-filings PDF text into a structured CSV.

Input:  output/ambest_nm_text.txt (extracted via pypdf)
Output: tools/ambest_nm_data.csv
...
INPUT_TXT = Path("output/ambest_nm_text.txt")
OUTPUT_CSV = Path("tools/ambest_nm_data.csv")

# Filing block end marker
END_MARKER = "Further information may be available for this filing"

# Header line — appears once per block: starts with "Approved NM MM/DD/YY"
APPROVED_RE = re.compile(r"^Approved NM (\d{2}/\d{2}/\d{2})", re.MULTILINE)
...
def split_filings(text: str) -> list[str]:
    ...
    return [p.strip() for p in parts if "Approved NM" in p or "Approved" in p]
