...
const sf9847c = src("State Farm Personal Car Policy — WA form 9847C & OR form 9837B", "https://www.statefarm.com/content/dam/sf-library/en-us/pca-endorsement/auto/9847C.pdf");
...
const SCENARIOS: Line = {
  ...
  features: [
    ...
    }, { statefarm: { category: "included", value: "Covered · State Farm is primary", confidence: "high", note: "Form 9847C: a permissive user of your car is an insured (Insured def. §3, within the scope of your consent), and State Farm is PRIMARY on your car (Other Insurance §2). No permissive-use step-down in the base policy.", source: sf9847c, scenario: { extends: ["BI", "PD", "MedPIP", "collision", "comprehensive"], limits: "Full policy limits — no step-down in the base form", deductible: "Same as the owned auto", primacy: "primary" } }, geico: { category: "included", value: "Covered · no step-down", confidence: "high", note: "GEICO WA form A30WA (12-22): a permissive user is an insured (Persons Insured, owned auto §2); no step-down (Limits of Liability). NOTE the difference from State Farm — GEICO's Other Insurance clause makes its coverage EXCESS over any other applicable liability insurance, rather than explicitly primary on your car.", scenario: { extends: ["BI", "PD", "MedPIP", "collision", "comprehensive"], limits: "Full policy limits — no step-down", deductible: "Same as the owned auto", primacy: "primary", stateExceptions: "GEICO's other-insurance clause is written excess over any other applicable insurance the permissive driver carries." } } }) },
    ...
    }, { statefarm: { category: "included", value: "Resident relatives are insureds", confidence: "high", note: "Form 9847C makes resident relatives insureds (Liability Insured def. §1), so a resident driver is covered even if unlisted. An undisclosed driver is a rating / material-misrepresentation matter (re-rating or a Conditions defense), not a liability exclusion.", source: sf9847c, scenario: { extends: ["BI", "PD", "MedPIP", "collision", "comprehensive"], limits: "Policy limits (resident relatives are insureds)", deductible: "Same as the owned auto", primacy: "primary", stateExceptions: "Undisclosed-driver / misrepresentation handling still varies by state." } }, geico: { category: "included", value: "Relatives are insureds", confidence: "high", note: "GEICO A30WA Persons Insured §1 (you and your relatives) — a resident relative is an insured even if unlisted. An undisclosed driver is a rating / misrepresentation matter, not a liability exclusion.", scenario: { extends: ["BI", "PD", "MedPIP", "collision", "comprehensive"], limits: "Policy limits (relatives are insureds)", deductible: "Same as the owned auto", primacy: "primary", stateExceptions: "Undisclosed-driver handling still varies by state." } } }) },
    ...
    }, { statefarm: { category: "varies", value: "Excluded driver = no coverage (all coverages)", confidence: "high", note: "State Farm WA 'Driver Exclusion Endorsement' (form 2023Q): while any vehicle is operated by the excluded person, no liability or obligation attaches under ANY coverage of the policy (BI, PD, loss of earnings, etc.), and that person is not an insured for PIP, Medical Payments, or UM/UIM (BI + PD). Whether a state permits the exclusion still varies.", source: src("SERFF: State Farm WA form 2023Q (Driver Exclusion Endorsement)", "https://serff-sfa.naic.org/serff/sfa"), scenario: { extends: [], limits: "None while the excluded driver operates any vehicle", deductible: "n/a", primacy: "n/a", stateExceptions: "State Farm files a driver-exclusion endorsement where the state allows it (WA does); several states prohibit/restrict exclusions." } } }) },
    ...
    }, { statefarm: { category: "included", value: "Covered · excess over owner · your deductible", confidence: "high", note: "Form 9847C: you/resident relatives are insured using a non-owned car (Insured §1.b), and State Farm is EXCESS (Other Insurance §3). Physical damage extends to a non-owned car IF you carry comp/collision (Covered Vehicle §5), at the lowest deductible on your schedule. A car possessed 31+ consecutive days stops being a 'non-owned car'.", source: sf9847c, scenario: { extends: ["BI", "PD", "MedPIP", "UMUIM", "collision", "comprehensive"], limits: "Your policy limits (excess over the owner's primary)", deductible: "Your lowest scheduled deductible (if you carry the coverage)", primacy: "excess", stateExceptions: "No extension once you've had the car 31+ consecutive days (then it isn't a 'non-owned car')." } }, geico: { category: "included", value: "Covered · excess over owner · your deductible", confidence: "high", note: "GEICO A30WA: on a non-owned auto you and relatives are insured with the owner's permission (Persons Insured, non-owned §1); GEICO is EXCESS (Other Insurance). Physical damage extends only if you carry comp/collision (a premium shown), at your deductible. Excluded if the car is furnished for your regular use (non-owned-auto definition).", scenario: { extends: ["BI", "PD", "MedPIP", "UMUIM", "collision", "comprehensive"], limits: "Your policy limits (excess over the owner's primary)", deductible: "Your deductible (if you carry the coverage)", primacy: "excess", stateExceptions: "A car furnished for your regular use isn't a covered 'non-owned auto'." } } }) },
    ...
    }, { statefarm: { category: "included", value: "Covered like your own car · your deductible", confidence: "high", note: "Form 9847C: a rental (under 31 days) is a 'non-owned car' — liability extends as EXCESS (Other Insurance §3), and comp/collision extend IF you carry them (Covered Vehicle §5) at your lowest scheduled deductible. Territory is the US and Canada. Loss of use / diminished value to the rental company are limited under physical damage. Distinct from Rental Reimbursement.", source: sf9847c, scenario: { extends: ["BI", "PD", "MedPIP", "UMUIM", "collision", "comprehensive"], limits: "Your policy limits", deductible: "Your lowest scheduled deductible; requires you to carry collision", primacy: "excess", endorsement: "Car Rental & Travel Expenses coverage handles some rental costs.", stateExceptions: "Loss-of-use / diminished-value recovery to the owner is limited.", seeAlso: "rental" } }, geico: { category: "included", value: "Covered · excess · your deductible", confidence: "high", note: "GEICO A30WA: a one-off rental is a non-owned auto — liability extends (excess), comp/collision extend if you carry them (a premium shown) at your deductible; territory is the US/Canada. DIFFERENCE vs State Farm: a vehicle REGULARLY rented on a daily/weekly/monthly basis is excluded unless declared (Liability excl. 19).", scenario: { extends: ["BI", "PD", "MedPIP", "UMUIM", "collision", "comprehensive"], limits: "Your policy limits", deductible: "Your deductible; requires you to carry collision", primacy: "excess", endorsement: "A regularly-rented vehicle must be declared to be covered.", stateExceptions: "Regularly rented (daily/weekly/monthly) vehicles are excluded unless scheduled.", seeAlso: "rental" } } }) },
    ...
    }, { statefarm: { category: "none", value: "Not covered outside US/Canada", confidence: "high", note: "Form 9847C 'Where Coverage Applies' §2: coverage applies only in the US (and its territories/possessions) and Canada — a rental driven elsewhere isn't covered; Mexico has a narrow separate 'Limited Coverage in Mexico' only. Business use of a non-private-passenger vehicle is also excluded (Liability excl. 9).", source: sf9847c, scenario: { extends: [], limits: "None outside the US/Canada territory", deductible: "n/a", primacy: "n/a", stateExceptions: "Limited Coverage in Mexico is a narrow exception; buy local/rental coverage abroad." } }, geico: { category: "none", value: "Not covered outside US/Canada", confidence: "high", note: "GEICO A30WA 'Policy Period; Territory': coverage applies only in the US (and its territories/possessions) and Canada. Ride-sharing and carrying persons/property for compensation are excluded (Liability excl. 17); a business/corporation-owned vehicle needs declaring (excl. 21).", scenario: { extends: [], limits: "None outside the US/Canada territory", deductible: "n/a", primacy: "n/a", stateExceptions: "No Mexico/overseas coverage; buy local/rental coverage abroad." } } }) },
    ...
    }, { statefarm: { category: "varies", value: "Resident family yes · non-family friend no", confidence: "high", note: "Form 9847C: on a non-owned car (the rental) only you and resident relatives are insureds (Insured §1.b). The permissive-others grant (§3) covers only your car / newly-acquired / temp-substitute — NOT a non-owned car — so a non-family friend driving your rental is not an insured under your policy.", source: sf9847c, scenario: { extends: ["BI", "PD", "MedPIP", "collision", "comprehensive"], limits: "Your limits for you / resident relatives; none for a non-family driver", deductible: "Your lowest scheduled deductible", primacy: "excess", stateExceptions: "Confirm additional-driver rules on the rental agreement too." } }, geico: { category: "varies", value: "Relatives yes · non-family friend no", confidence: "high", note: "GEICO A30WA: on a non-owned auto only you and your relatives are insureds (Persons Insured, non-owned §1). A non-family friend driving your rental is not an insured under your policy.", scenario: { extends: ["BI", "PD", "MedPIP", "collision", "comprehensive"], limits: "Your limits for you / relatives; none for a non-family driver", deductible: "Your deductible", primacy: "excess", stateExceptions: "Confirm additional-driver rules on the rental agreement too." } } }) },
    ...
    }, { statefarm: { category: "none", value: "As owner: excluded · as renter: non-owned-car coverage", confidence: "high", note: "Renting your car OUT is excluded: WA form 9847C names it ('PERSONAL VEHICLE SHARING, PEER-TO-PEER CAR SHARING', Liability excl. 5); OR form 9837B uses the general 'rented to or leased to others' exclusion — same result. As a RENTER, a P2P car you rent is a 'non-owned car', so State Farm's non-owned coverage applies (liability excess; physical damage if you carry it); the platform's plan is primary. Distinct from Rideshare.", source: sf9847c, scenario: { extends: [], limits: "None as owner; non-owned-car limits (excess) as renter", deductible: "Your lowest scheduled deductible (renter side)", primacy: "excess", endorsement: "No endorsement covers renting your car out.", stateExceptions: "The platform's coverage is primary on the renter side.", seeAlso: "rideshare" } }, geico: { category: "none", value: "As owner: excluded · as renter: non-owned coverage", confidence: "high", note: "GEICO A30WA Liability excl. 22 excludes an owned auto 'leased or rented to others or given in exchange for any compensation, including... peer-to-peer vehicle sharing during the vehicle sharing period' (with detailed P2P definitions §7–8) — renting your car OUT is excluded. As a RENTER, a P2P car is a non-owned auto, so GEICO's non-owned coverage applies (excess); the platform's plan is primary. Distinct from Rideshare.", scenario: { extends: [], limits: "None as owner; non-owned-auto limits (excess) as renter", deductible: "Your deductible (renter side)", primacy: "excess", endorsement: "No endorsement covers renting your car out.", stateExceptions: "The platform's coverage is primary on the renter side.", seeAlso: "rideshare" } } }) },
    ...
    }, { statefarm: { category: "none", value: "Regular-use / heavy / non-PPC excluded", confidence: "high", note: "Form 9847C: a company car furnished for your regular use stops being a 'non-owned car' once possessed 31+ consecutive days (no extension). 'Private Passenger Car' is capped at 10,000 lbs GVWR; a truck-tractor isn't a 'car'; business use of a non-private-passenger vehicle is excluded (Liability excl. 9); non-owned trailers are narrowly defined.", source: sf9847c, scenario: { extends: [], limits: "Excluded for regular-use/company and over-10,000-lb vehicles", deductible: "n/a", primacy: "n/a", endorsement: "A drive-other-car / extended non-owned endorsement can add a company car.", stateExceptions: "GVWR and vehicle-type definitions per form 9847C." } }, geico: { category: "none", value: "Regular-use / business / non-PP excluded", confidence: "high", note: "GEICO A30WA: a business/corporation-owned vehicle is excluded unless declared (Liability excl. 21); a non-owned auto furnished for your regular use isn't covered; a covered 'non-owned auto' is a private passenger, farm, utility auto or trailer, so heavier commercial vehicles fall outside; a vehicle leased under 6 months needs declaring (excl. 18).", scenario: { extends: [], limits: "Excluded for regular-use/company and non-private-passenger vehicles", deductible: "n/a", primacy: "n/a", endorsement: "Declare a company/business vehicle to cover it.", stateExceptions: "Vehicle-type definitions per form A30WA." } } }) },
  ],
};
