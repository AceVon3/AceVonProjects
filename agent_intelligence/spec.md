...
- **Setup nexus-threshold panel — PARKED 2026-07-14** (user decision: not yet confident in the section; do not render until the user asks for it back). The component (`NexusThresholdPanel`) and its verified 50-state data (`economicNexus.ts`) remain in the codebase and `verify_office_summary` keeps validating the data; `e2e_setup` pins the panel's ABSENCE. Original design (for reintroduction) — (2026-07, `economicNexus.ts` + `NexusThresholdPanel` under the licensed-states picker): as selling states are selected, shows each state's income/franchise/gross-receipts tax economic-nexus revenue line for an out-of-state service business (insurance-agency commissions) — NOT sales-tax Wayfair thresholds. Per state: basis classification (factor-presence bright line with dollar figure / gross-receipts tax line / doing-business no-bright-line / no-income-tax), indexed marker, official source link, load-bearing note (e.g. TX $500k nexus vs. indexed no-tax-due floor; OH CAT $6M from 2025; CA thresholds not a safe harbor; AR $250k new for 2026). Figures are verified product copy that changes (several indexed annually) — `economicNexus.ts` is the single update point; the panel always carries the confirm-with-a-tax-professional hedge and never says whether the agent's own book crosses a line.
- **Out-of-state remote registration guide** (2026-07, `stateRegistration.ts` + the `remote-registration-guide` section in `OfficeSummary`): renders when `remote_count > 0` AND at least one employee state ≠ the primary office state. Explains that a remote employee generally brings their state's payroll obligations (withholding, UI, workers' comp) regardless of agency size, and that payroll software (QuickBooks etc.) calculates/files another state's taxes only AFTER the employer obtains that state's withholding account number and unemployment account number — the registration step software does not do and small employers most often miss. Per out-of-state state: official registration links (withholding + unemployment + one-stop portal where one exists). The nine no-wage-income-tax states (AK FL NV NH SD TN TX WA WY) render a "no withholding account" note instead of a withholding link. These are USER-FACING browser links (never generator-fetched): official government domains only, but bot-blocked-to-fetchers pages are acceptable. Copy stays relevance-pointing ("generally", "typically", confirm-with-professional close); size-independence is stated explicitly.
