...
def verify(con: sqlite3.Connection, rolled_count: int, multi_count: int) -> None:
    ...
    ok = raw == 6045
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (1) scraped filings_raw rows: expected 6045, got {raw}")
    ...
    ok = rolled == 3756
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (3) scraped filings (rolled) rows: expected 3756, got {rolled}")
    ...
    ok = n_states == 36
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (8) scraped distinct states: expected 36, got {n_states}")
    ...
    ok = raw_scraped == 6045 and raw_ambest > 0
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (11) filings_raw source tags: 6045 serff_scraped "
    ...
    ok = f_scraped == 3756 and f_ambest > 0
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (12) filings source tags: 3756 serff_scraped "
