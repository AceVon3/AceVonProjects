...
# PERMANENT (non-SERFF, never replaceable): CA (CDI) runs its own filing
# system, not SERFF Public Access. Conservative-safe tag: mis-tagging a
# scrapeable state permanent just means "revisit manually"; mis-tagging a
# non-scrapeable state interim lets the replacement sweep delete it with no
# replacement (the CA landmine).
# DOCTRINE UNWIND (2026-07-28/30): NY and TX were on this list from regulatory
# structure alone, never probed. Probes proved BOTH live on SERFF Filing
...
AMBEST_STATES = [  # VA (06-22), OH (06-24), IL (06-25), WV (06-26), NH (06-29), VT (06-30), HI (07-01), AK (07-05), DE (07-08) all removed: scraped (serff_scraped), no longer interim.
                 ...
                 # KS (07-09), NJ (07-10), MS (07-10) removed: scraped — 26th/27th/28th scraped states.
                 # IA (07-10), SC (07-10) removed: scraped — 29th/30th scraped states.
                 ...
                 # MO (07-21) removed: scraped — 40th scraped state.
                 # MN (07-22) removed: scraped — 41st scraped state.
                 # TN (07-22) removed: scraped — 42nd scraped state, chain
                 # complete. AMBEST_STATES is now PERMANENT-only (CA/NY/TX).
                 # TX (07-30) removed: scraped — 45th scraped state (701/701
                 # targets in 26 bursts on the 5-min cadence, 510-row
...
_AMBEST_EXCLUDE = ("american family home", "american modern", "homesite", "midvale",
                   ...
                   "economy premier", "economy preferred",
                   # Metropolitan P&C: the legacy-MetLife Auto&Home vehicle Farmers
                   # acquired (2021) — same legacy-MetLife family as economy_*,
                   # renewal-book profile (Peerless doctrine). 2026-07-10, KS.
...
def derive_brand(name) -> str | None:
    ...
    if n.startswith("encompass"):
        return "Encompass"
    # TravCo Insurance Company + The Phoenix Insurance Company — Travelers
    # subsidiaries whose legal names lack "travelers"; they co-file on TRVD-*/
    # TRVD-G* MA Quantum Home filings. MA-first (2026-07-06): 0 rows in the 18
    # prior scraped states (verified), so these cannot shift shipped brands.
    # Farmington Casualty Company + The Charter Oak Fire Insurance Company —
    # same class, NY-first (2026-07-30): both co-file on TRVD-*/TRVD-G* NY
    # filings alongside the full Travelers stack; 0 rows in the 44 prior
    # scraped states (verified), so these cannot shift shipped brands.
    if ("travelers" in n or "travco" in n or n.startswith("the phoenix insurance")
            or "farmington casualty" in n or "charter oak fire" in n):
        return "Travelers"
    # "drive new jersey": Progressive's NJ-specific consumer auto writer (a
    # DISTINCT live entity, not the retired national Drive brand the exclude
    # list names as "drive insurance") — found at the NJ import 2026-07-10.
    # "american strategic": ASI = Progressive Home.
    # "mountain laurel": Mountain Laurel Assurance Company — Progressive
    # subsidiary (its TN/PA consumer-auto vehicle) that co-files on PRGS-*
    # filings; legal name lacks "progressive". TN-first (2026-07-22): 0 rows
    # in the 41 prior scraped states (verified against the shipped
    # all_states csv), so this mapping cannot shift shipped brands.
    if n.startswith("progressive") or "artisan and truckers" in n or "drive new jersey" in n or "american strategic" in n or "mountain laurel" in n:
        return "Progressive"
    ...
    # Wausau Underwriters Insurance Company — Liberty-owned (Wausau Insurance
    # Companies, acquired 1999; brand retired). Co-files on Liberty Mutual MA
    # HO filings (LBPM-133878161) alongside Liberty Mutual Ins/Personal +
    # Montgomery. MA-first (2026-07-06): 0 rows in the 18 prior scraped states
    # (verified), so this mapping cannot shift any shipped state's brands.
    # Wausau family: EXCLUDED (Decision 2, 2026-07-06), not mapped — retired
    # brand, no consumer channel, same LM-filing-vehicle profile as Peerless/
    # LM P&C (excluded on the same MA filing). See _AMBEST_EXCLUDE. NOTE:
    ...
    # independents.
    # Colonial County Mutual Insurance Company — Nationwide's Texas county-
    # mutual auto vehicle; all 7 TX rows co-file on NWPP-* filings alongside
    # Nationwide Mutual (same class as Liberty County Mutual -> LM). TX-first
...
def verify(con: sqlite3.Connection, rolled_count: int, multi_count: int) -> None:
    ...
    # Re-keyed 2026-08-10 (26h2 REFRESH import — first full-dataset refresh):
    # 9711 -> 10585 = +877 refresh rows across 44 states (every state >= 0,
    # zero losses, 0 changed values; TX +70, GA +61, NV +45 lead; 485 nonzero-
    # impact; 29 eff-2027 rows admitted by the EFFECTIVE_DATE_TO bump) - 3
    # exhibit debris (CA GECC-134953695 GEICO '1/2/3 year trend' rows, killed
    # by the new trend guard in the scraper's _FS_EXHIBIT_DEBRIS_RE; the
    # filing's real GEICO Protection +6.9% row kept). Includes the shape-K
    # recovery (GA CFPC-134983942 COUNTRY Preferred 0%/58,678 ph). TDI TX +
    # CDI CA standing audits both clean pre-import (CDI 7/31 list; 3 SF-CA
    # HO still pending-with-waiver, recheck at the 8/31 list).
    raw = con.execute("SELECT COUNT(*) FROM filings_raw WHERE source='serff_scraped'").fetchone()[0]
    ok = raw == 10585
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (1) scraped filings_raw rows: expected 10585, got {raw}")
    ...
    ok = rolled == 6642
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (3) scraped filings (rolled) rows: expected 6642, got {rolled}")
    ...
    # 1776 as of 2026-07-30 (TX import). Attribution vs the HEAD (LA-era) db,
    # exact identity 1617 - 25 + 184 - 0 = 1776: -25 = the 3-day as_of slide
    # (07-27 -> 07-30; MI 12, CO 3, GA/PA 2...); +184 = TX 129 (new state) +
    ...
    # Anchor re-keyed at the TN import (2026-07-22): GMMX-134651170 is the
    # PDF-verified Encompass Personal Umbrella re-rate (+110.9% on a 1,490-ph
    # / $851k book) — it displaces the long-standing SFMA-134315091 WA +93.7%.
    ok = (serff == "GMMX-134651170" and abs(impact - 110.90) < 0.05
          and brand == "Encompass" and state == "TN")
    failed |= not ok
    ...
    ok = raw_scraped == 10585 and raw_ambest == 0
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (11) filings_raw source tags: 10585 serff_scraped "
    ...
    ok = f_scraped == 6642 and f_ambest == 0
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (12) filings source tags: 6642 serff_scraped "
