...
def verify(con: sqlite3.Connection, rolled_count: int, multi_count: int) -> None:
    ...
    ok = raw == 3821
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (1) scraped filings_raw rows: expected 3821, got {raw}")
    ...
    ok = rolled == 2342
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (3) scraped filings (rolled) rows: expected 2342, got {rolled}")
    ...
    ok = n_states == 25
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (8) scraped distinct states: expected 25, got {n_states}")
    ...
    ok = raw_scraped == 3821 and raw_ambest > 0
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (11) filings_raw source tags: 3821 serff_scraped "
    ...
    ok = f_scraped == 2342 and f_ambest > 0
    failed |= not ok
    print(f"  [{'OK' if ok else 'FAIL'}] (12) filings source tags: 2342 serff_scraped "
