...
//   - Validation table has 20 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 20 rows (got ${rowCount})`, rowCount === 20);
  ...
  // 5 not covered = 50 − 45 covered (20 directly scraped incl. VA/OH/IL/WV/NH/VT/HI/AK/MA/ND + 25 AM Best).
  // The uncovered 5: AL/FL/LA (header-only re-pull), NC (structural NCRB gap), WY
  // (no filings). The validation table is 20 rows (one per directly-scraped state;
