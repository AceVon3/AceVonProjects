...
//   - Validation table has 19 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 19 rows (got ${rowCount})`, rowCount === 19);
  ...
  // 5 not covered = 50 − 45 covered (18 directly scraped incl. VA/OH/IL/WV/NH/VT/HI/AK + 27 AM Best).
  // The uncovered 5: AL/FL/LA (header-only re-pull), NC (structural NCRB gap), WY
  // (no filings). The validation table is 19 rows (one per directly-scraped state;
