...
//   - Validation table has 28 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 28 rows (got ${rowCount})`, rowCount === 28);
  ...
  // (no filings). The validation table is 28 rows (one per directly-scraped state;
