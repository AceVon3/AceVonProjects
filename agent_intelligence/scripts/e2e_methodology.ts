...
//   - Validation table has 21 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 21 rows (got ${rowCount})`, rowCount === 21);
  ...
  // 5 not covered = 50 − 45 covered (21 directly scraped incl. VA/OH/IL/WV/NH/VT/HI/AK/MA/ND/RI + 24 AM Best).
  // The uncovered 5: AL/FL/LA (header-only re-pull), NC (structural NCRB gap), WY
  // (no filings). The validation table is 21 rows (one per directly-scraped state;
