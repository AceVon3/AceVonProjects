...
//   - Validation table has 36 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 36 rows (got ${rowCount})`, rowCount === 36);
  ...
  // (no filings). The validation table is 36 rows (one per directly-scraped state;
