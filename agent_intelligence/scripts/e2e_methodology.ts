...
//   - Validation table has 40 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 40 rows (got ${rowCount})`, rowCount === 40);
  ...
  // (no filings). The validation table is 40 rows (one per directly-scraped state;
