...
//   - Validation table has 30 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 30 rows (got ${rowCount})`, rowCount === 30);
  ...
  // (no filings). The validation table is 30 rows (one per directly-scraped state;
