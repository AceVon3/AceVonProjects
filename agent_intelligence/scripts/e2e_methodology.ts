...
//   - Validation table has 41 rows (one per directly-scraped state) and the cell
...
async function main(): Promise<void> {
  ...
  check(`validation table has 41 rows (got ${rowCount})`, rowCount === 41);
  ...
  // (no filings). The validation table is 41 rows (one per directly-scraped state;
