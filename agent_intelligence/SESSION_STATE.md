...
- **50-STATE DATA: 516/562 (state,topic) pairs grounded** via the fetch-and-
  ground pipeline (12-batch URL harvest, per-URL verified; 5 kill-safe --only
  regen chunks via scripts/list_pairs.ts — env kills bg shells ~30min and the
  generator writes only at END; a full run lost 369 summaries once). Generator
  hardening: browser-UA headers, curl.exe fallback (Node-TLS-quirk hosts:
  cga.ct.gov, dor.ms.gov, legislature.vermont.gov), pdf-parse v2 extraction
  BOTH fetch paths (unlocked CO PAY CALC, NV/WI at-will, MS workers-comp).
  HONEST REMAINDERS (46): remote×20 (no such law — product decision pending),
  AR×5 + AZ/remote + TX/payroll (hard WAF blocks), CO at-will (doctrine page
  403s), termination/hiring/OH thin-page cluster (~19, statute-hunt fixable).
  At-will pairing insight: generator needs doctrine AND exceptions halves —
  pair statute/court-opinion/agency-FAQ with the civil-rights agency page.
- **STATE_CONFIG (briefing.ts) = single home of threshold product copy**: 26
  size gates across 24 states (leave, retirement mandates, wage quirks), MT
  WDEA not-at-will, own-threshold salary warnings (CA/NY/CO/AK/ME/HI).
  Per-state review blocks in the office summary derive from the SAME config.
- **3 TAX SURFACES (user-requested, from the QuickBooks/nexus Q&A):**
  (1) remote-registration guide (OfficeSummary amber callout: nexus explainer
  + 50-state withholding/UI registration links, stateRegistration.ts; 9
  no-wage-income-tax states get no-withholding notes); (2) setup
  NexusThresholdPanel (economicNexus.ts: 50-state income/franchise-tax
  economic-nexus lines — NOT Wayfair sales-tax; fresh-law catches: OH CAT
  $6M 2025+, AR $250k new 2026, NJ $100k/200, NY $1,283,000 indexed, CA
  $757,070 indexed non-safe-harbor, NV $4M flat, KY no-minimum); (3) nexus
  explainer copy. INDEXED figures (CA/NY/AL + PAY CALC etc.) need an annual
  recheck — candidates for a scheduled regen routine (user aware, not set up).
- **Tests self-adapt**: e2e_compliance derives card expectations from
  complianceData itself; determination-language blocklists cover review lines
  (50 states × N=1/15/60), reg guide, nexus panel. Full gates green at ship:
  tsc, 8/8 verify, build 12/12, 13/13 e2e.
- **Deploy flow gotchas**: subtree split can collide with direct GitHub web
  uploads to agent_intelligence (user uploaded a logo zip — absorbed into the
  monorepo subtree, one-time --force-with-lease reconcile; content preserved).
  PowerShell `>` corrupts binaries — use cmd /c redirects. Verify deploys by
  grepping PROD chunks for a feature marker (data may land in SHARED chunks,
  not the page chunk).
- **HEADs (this arc)**: monorepo fe10f54..f971ecb (compliance commits);
  agent-intel/master deployed through f971ecb (nexus panel live-verified);
  remote has since moved with the parallel scraper session's deploys.

---
# Session checkpoint â€” 2026-07-10 (mega pass)  Â·  KS/NJ/MS IMPORTED (26th-28th) + B16 12-STATE CORRECTION SHIPPED
...
adjudication), build, **13/13 e2e in a clean git worktree** â€” the deploy was
...
# Session checkpoint â€” 2026-07-09 (CT + B14)  Â·  CT IMPORTED (25th state) + B14 FLEX-RATE GATE FIXED + B13/B14 AUDITS CLOSED EMPTY
...
- **B14 (4th filing-type-vocabulary variant):** flex-rating CT files "Flex
  Rate" â†’ the Rate/Rate-Rule gate silently dropped LIVE PPA signals (GEICO
  ...
  drift scan structurally cannot see gate bugs â€” same gate both sides).
  ...
  6,689 PDFs, 0 sightings, 0 delta, all 50 funnels balance â€” CLOSED EMPTY
  (scraper repo B14_SWEEP.md).
- **B13 (deliverable drift) CLOSED EMPTY:** all 25 shipped states
  re-finalized offline vs shipped deliverables â€” 0 live-material drift.
  ...
  re-finalize.
- **CT cross-check (15th interimâ†’real point):** value-agreement 55/56
  ...
  / 1 adjudicated new-product), 0 genuine soft-misses; Farmers enrichment Ã—2
  (AM Best lacks â€” incl. FARM-134733599 +6.0%/5,133ph eff 04/14/26, serves
  on PROD). validated:{auto:true, home:true}; methodology 24â†’25.
- **Import gate ALL PASS:** 24 prior scraped states byte-identical; CA/NY/TX
  intact; delta == exactly CT (+137 raw / +95 rolled / +29 active â€” the +29
  ...
  25 scraped CT âœ“âœ“; CT Prospect 15 rows live incl. B14-recovered
  GECC-134763927 (+5.83% weighted); PRGS-134767624 rolls ~+3.6% (below
  Prospect â‰¥5) â€” in data, correctly not a Prospect row.
- **HEADs (post-deploy):** insurancewebscraper `3f34590`â†’**`71c2a9b`**
  (HEAD==origin, 0 deletions). agent-intel/master `6f9873c`â†’**`79b4ec2`**
...
- **B12 update:** environmental kill-waves hit background tasks (incl. sleep
  ...
  `next start` held filings.db locked at import (killed with approval) â€”
  shut localhost servers down at checkpoint close.

---

# Session checkpoint â€” 2026-07-08 (DE + rda-gate)  Â·  DE IMPORTED (24th state) + THE 3rd SILENT-DROP CLASS CORRECTED (None-conflation, multi-state)
...
- **THE 3rd MATERIAL SILENT-DROP CLASS (after B6 comma, B9 mapping):** a
  three-valued-logic bug â€” the funnel's `if not fs.rate_data_applies`
  ...
  does-NOT-apply â†’ fully-parseable rate filings silently dropped. **Fixed:**
  ...
  all-state scan â†’ 11 candidates â†’ funnel-gate adjudication.
- **DAMAGE PROFILE:** DE (unshipped) held ALL the live damage â€” SF
  **âˆ’1.5%/244,139ph ACTIVE** (renders in My Carriers), Encompass **+8.6%**
  + Allstate **+17.5%** (both LIVE Prospect rows now), the Nationwide
  24,671ph trio, + a bonus Travelers +6.7%/15,911 back-ext â€” caught
...
- **MULTI-STATE GATE ALL PASS:** exactly 5 states changed by exact deltas
  (MT +2/+1/0 via SURGICAL APPEND â€” the diff-gate caught MT's enrichment
  fields and auto-restored, the B6 lesson working; WV +2/+2/0; IL +1/+1/**+1
  active** â€” a rate-neutral row, count+1/signal+0 verified; OH +4/+2/0;
  ME +1/+1/0); 18 unaffected scraped states byte-identical; CA/NY/TX
  intact; as-of pinned. DE flip exact (190 interim â†’ 133/67/20).
- **DE cross-check (14th point, post-fix):** value-agreement 64/64 (100%),
  interim 57/57, PPA 98.3% / HO all-recency â†’ in-window 100%/100%.
  validated:{auto:true, home:true}; methodology 24 rows DE âœ“âœ“. Gates: tsc,
  ...
  identical).
- **â˜… B13 OPENED â€” THE IMMEDIATE NEXT SESSION (before CT, before the large
  tier): DELIVERABLE DRIFT.** MT's diff-gate exposed 5 filings CACHED AFTER
  MT SHIPPED that a re-finalize would now include â€” one LIVE-MATERIAL:
  **Allstate +20.1%/642ph eff 04/06/2026** (in a shipped state's cache, not
  ...
  shipped) states â€” per drifted filing MATERIAL+LIVE (recover) vs back-ext/
  immaterial; full B6 protocol; HOLD for scope review.
- **HEADs (post-deploy):** insurancewebscraper `ac8b5e5`â†’â€¦â†’**`3f34590`**
  (DE arc + the correction; HEAD==origin, 0 deletions). agent-intel/master
  `df4c2ee`â†’**`6f9873c`** (subtree-FF, 0 deletions; Vercel LIVE â€” DE âœ“âœ“,
  methodology 24, recovered signals serve from PROD). Monorepo
  `0ba260f`+docs. `filings.db` **`2b9316cf`**. CT swept + parked (22/27,
  retry pending â€” AFTER B13).

---

# Session checkpoint â€” 2026-07-08 (SD)  Â·  SD INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (23rd scraped state; THE COVERAGE THESIS SEALED)

SD's 121-row SERFF scrape (250/250 in-target, 0 download misses â€” 13-state
...
- **THE COVERAGE THESIS SEALED:** SD â€” squarely in COUNTRY's plains
  footprint â€” resolves RICHNESS-ONLY: COUNTRY's entire windowed SERFF
  presence is ONE Personal-Farmowners FORM filing (CFPC-133997117), zero
  PPA/HO rate filings â†’ **AM Best's 0 CORRECT in a genuine Country market.
  AK is DEFINITIVELY the lone thin case across 13 interimâ†’real states; its
  ...
  hardest counterfactual â€” the basis for trusting the ~19 remaining interim
  states' coverage.
- **CROSS-CHECK (13th point):** value-agreement 66/67 â†’ **67/67 adjudicated**
  (the differ = AM Best carrying BOTH stages of one Safeco filing â€” original
  âˆ’6.0% + 0.0% amendment; our âˆ’6.0% == the source PDF; a comparator pairing
  artifact, NOT a 4th AM-Best-side error); interim 58/59 â†’ **59/59**;
...
- **B9 SHOWCASE STATE:** the sweep's 190 "farmers" rows contained **121
  ...
  34) â†’ decisive None, 0 in targets, 0 in the deliverable** â€” pre-B9, SD
  ...
  Connect/Munich-Re â€” the first big AmFam state post-correction). American
...
- **SHAPE-L found + backlogged:** LBPM-134438278 (0.000%/5,597ph,
  blank-premium-change cell â€” data-present-unparsed but rate-neutral,
  ...
  5 members (H/I/J/K/L) â€” **the eventual deliberate fix should GENERALIZE
  to the blank/zero family, not enumerate another shape.**
- **LARGE-TIER PLAYBOOK VALIDATED (SD's 6-burst cap experiment):** caps
  12â†’15â†’18 all clean-to-cap on 15-min rests (yield ~linear, challenges flat
  ~20%); burst-1's sustained wall recovered on ONE 15-min rest â†’ **single
...
- **Gate ALL PASS:** 22 prior scraped byte-identical; CA/NY/TX intact; SD
  flip exact (+121/+75/+28; ambest âˆ’145/âˆ’86; 0 remnants); as-of pinned.
- **HEADs (post-deploy):** insurancewebscraper `b2e1fe1`â†’`f06bb8f`(universe)
  â†’6 burst banksâ†’`b45cc33`(collection)â†’**`27c3f00`** (all_states + compare;
  HEAD==origin, 0 deletions throughout). agent-intel/master
  `ff3634d`â†’**`df4c2ee`** (subtree-FF, 0 deletions; Vercel LIVE â€” SD âœ“âœ“,
  methodology 23). Monorepo `2528774`+docs. `filings.db` **`bd36ba67`**.
- **NEXT: DE (recon DONE, pipelined)** â€” 190 in-scope == db interim, 10
...
# Session checkpoint â€” 2026-07-08 (ME)  Â·  ME INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (22nd scraped state; the ISOLATION-BOUNDARY verdict)

ME's 114-row SERFF scrape (309/309 in-target, 0 true misses â€” 12-state streak)
...
- **CROSS-CHECK (12th interimâ†’real point â€” CLEANEST EVER):** value-agreement
  **48/48 = 100% (first perfect)**; interim **46/46 = 100%, 0 disagreements**;
  ...
  one LBPM pair â†’ **adjudicated in-window 100%/100%**. **3rd CONSECUTIVE
  AM-Best-side error corrected at source** (ME LBPM-134490019 pre-final stage â€”
  ...
  precedent) â†’ validated:{auto:true, home:true}.
- **ISOLATION-BOUNDARY VERDICT â€” pattern SHARPENED:** ME (small/rural/
  ...
  its 1 material â€” Quantum +7.5%/16,008ph â€” IS in AM Best). **Thin = a major
  regional writer AM Best undercounts (AK's Country, 16Ã—). AK is the LONE thin
...
- **Funnel (balances, no_pdf=0):** 309 = 81 emitted + 191 form/rule + 11
  new-product + 4 rate-n/a + 22 out-of-window; 119 rows âˆ’ 5 filing-vehicle =
  ...
  live PPA/HO rate filings) â†’ decisive None, 0 in targets, 0 in deliverable â€”
  **the tripwire's first live catch** (pre-B9 = phantom COUNTRY rows in a
  state COUNTRY doesn't write). Munich-Re AmFam (45 sweep rows) excluded âœ“.
...
- **Gate ALL PASS:** 21 prior scraped states byte-identical; CA/NY/TX intact;
  ME flip exact (+114/+79/+20; ambest âˆ’131/âˆ’91; 0 remnants); as-of pinned
  PROACTIVELY (the RI mtime lesson applied â€” import passed first run).
- **HEADs (post-deploy):** insurancewebscraper `81aba34`â†’`6007b47`(universe)
  â†’`508a665`/`31fe306`/`43d0d76`/`5744b9a`/â€¦/`b75db3b`(7 burst banks +
  collection)â†’**`b2e1fe1`** (all_states + compare; HEAD==origin, 0 deletions
  throughout). agent-intel/master `61b7f2c`â†’**`ff3634d`** (subtree-FF, 0
  deletions; Vercel LIVE â€” ME âœ“âœ“, methodology 22). Monorepo `983b8f6`+docs.
  `filings.db` **`e28c47fc`**.
- **NEXT: SD (recon DONE, pipelined)** â€” **the SHARPEST boundary test:
  ...
  Liberty-brand absent but Safeco present â€” plains channel, confirm-at-sweep;
...
# Session checkpoint â€” 2026-07-07 (RI)  Â·  RI INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (21st scraped state; same-day full arc on the adaptive cadence)

RI's 100-row SERFF scrape (193/193 in-target, 0 true misses â€” 11-state streak)
replaced its AM Best interim; full arc IN ONE DAY: recon â†’ sweep (walled 5
terms same-day, rested retry clean) â†’ TravCo gap found+fixed globally â†’
5-burst harvest â†’ cross-check â†’ import â†’ deployed. db md5 **`5ce58df3`**
...
- **Gate ALL PASS:** 20 prior scraped states BYTE-IDENTICAL; CA/NY/TX intact
  ...
  +20 active; ambest âˆ’119/âˆ’62; 0 remnants); as-of pin held (one self-caught
  slip: the all_states regen reset the xlsx mtime â†’ check (9) caught the slid
  window â†’ re-pinned 2026-06-11, rebuilt byte-identical).
- **CROSS-CHECK (11th interimâ†’real point):** PPA 39/42 (92.9%) / **HO 24/25
  (96.0%) on ROBUST N** (coastal HO, the real home read â€” no small-N caveat);
  **adjudicated in-window 100%/100%**; value-agreement 41/42 â†’ **42/42** and
  interim 40/41 â†’ **41/41** after adjudicating 2 AM-BEST-SIDE errors at the
  source PDFs: (1) TRVD-G134527101 ENTITY TRANSPOSITION (AM Best swapped Home
  and Marine â†” TravCo values; ours matches the PDF â€” the VA SFMA pattern);
  (2) SFMA-134293558 PRE-REVISION STAGE (AM Best âˆ’1.0%/4,391 = the PDF's
  "Prior Value"; ours âˆ’0.9%/4,463 = the requested/current). Scrape
  source-true on both. 0 genuine soft-misses (2 AM-Best-only = 04/20/26
  recency). validated:{auto:true, home:true}.
- **COVERAGE VERDICT: RICHNESS-ONLY Ã—4** (Country/GEICO/Farmers/Encompass) â€”
  Encompass's 8 SERFF in-target = 6 form/rule + 2 new-product â†’ AM Best's 0
...
- **TRAVCO SEARCH-TERM GAP (found at recon, fixed globally):** a TravCo-led
  ...
  corroborated) was INVISIBLE to the bare "travelers" search â€” TravCo leads
...
- **PARSE FUNNEL (balances, no_pdf=0):** 193 = 55 emitted + 115 form/rule +
  11 new-product + 4 rate-data-n/a + 8 out-of-window + 0 no_pdf; 104 rows âˆ’
  4 filing-vehicle = 100. **0 blank max/min (cleanest parse yet)**; Munich Re
  AmFam collision 0 rows âœ“.
- **15-MIN REST TEST: PASSED at small-state load** â€” 15â€“17-min rests held
  ...
  predicted it) â€” rest length doesn't fix a depressed slow component â†’ bank.
...
- **B7/B7b: held through the 5-burst disk-heavy harvest** (checkout clean
  ...
  definitive proof still awaits a low-disk-trigger event. **B12 opened:** 2Ã—
  external task-kill at ~30-min age (mid-burst, both miss-safe) = possible
  environmental timeout â€” investigate before the large tier.
- **HEADs (post-deploy):** insurancewebscraper `d692e1f`â†’`5dad0a0`(universe)
  â†’`ea0340d`/`dfe32f5`/`865ac61`/`862f962`(burst banks)â†’`be692fe`(collection
  complete)â†’**`81aba34`** (all_states regen; HEAD==origin, 0 deletions
  throughout). agent-intel/master `e7debfe`â†’**`61b7f2c`** (subtree-FF, 0
  deletions; Vercel LIVE â€” RI renders, methodology 21 rows). Monorepo
  `a670f16`+docs. `filings.db` **`5ce58df3`**.
- **NEXT:** ME (71 AM Best in-target, next smallest) â€” recon can run offline
  anytime; then the large tier after B12 is understood.

---

# Session checkpoint â€” 2026-07-07 (B9 audit)  Â·  DERIVE_BRAND OVER-MATCH CLASS AUDIT COMPLETE + AMFAM-CONNECT INTERIM CORRECTION SHIPPED (61 rows / 23 states)

The 3rd class-audit (search-terms â†’ parser-shapes B6 â†’ mapping-rules B9): a
...
- **HONEST SCOPE:** the ask estimated ~240 live mislabels across 3 rule
  classes; measured reality = **61 live (AmFam Connect only)** â€” there is no
...
- **THE LIVE CORRECTION:** American Family Connect P&C (ex-Ameriprise, a
  DISTINCT excluded brand â€” the scraper's EXCLUDED_SUBSIDIARY_PATTERNS had it;
  ...
  **Correction gate ALL PASS:** all 20 scraped states BYTE-IDENTICAL; exactly
  23 states changed; every per-state delta == the adjudicated rows (MD âˆ’6,
  PA âˆ’5, CT/IN/MI/NE/TX âˆ’4, MN/MO/NJ/CA âˆ’3, KS/KY/MS/OK/TN/WI âˆ’2,
  AR/DE/IA/NY/SC/SD âˆ’1; raw == rolled); **first-ever consciously-approved
  permanent-state touch (CA âˆ’3 / NY âˆ’1 / TX âˆ’4)**; ambest 5,237â†’5,176 raw /
  3,323â†’3,262 rolled; as-of pin held. Footprint-verified: MD/PA/CT/MI had
  ONLY Connect (now 0 AmFam rows â€” AM Best had no genuine AmFam there);
  IN/NE/WI/MN/MO/KS genuine AmFam intact.
- **FORWARD HARDENING (852-name full-surface old-vs-new diff â€” 0 collateral,
  ...
  exactly 12 + 2. Bare "liberty" â†’ `_LM_LIBERTY_PATTERNS` allowlist (all 10
  live liberty names genuine â€” 0 live change). Connect/AFLAC/warranty guards
  decisive-None. Scraper: bare "farmers"/"country" REMOVED from GROUP_KW â€”
  ...
  guard CAUGHT REAL COLLATERAL: 5 genuine FIG entities (Columbus â€” live
  scraped OH; New Century/TX County Mutual/Lloyds â€” live ambest; Hawaii)
  would have been orphaned by removing the bare keyword â†’ enumerated
  ...
  own search term â€” **byte-identical-proven no-op** (re-import reproduced
  `5423a7c3` exactly).
...
- **HEADs (post-deploy):** insurancewebscraper `9645a69`->**`d692e1f`**
  ...
  (subtree-FF, 0 deletions; Vercel LIVE â€” 200s, methodology 20; **PROD==LOCAL
...
# Session checkpoint â€” 2026-07-07 (ND)  Â·  ND INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (20th scraped state) + B9 FARMERS-MISLABEL INTERIM CORRECTION

ND's 114-row SERFF scrape (290/290 in-target, 0 true misses â€” 10-state streak)
...
- **RESUME-FROM-INTERRUPTION NOTE:** the import step was interrupted mid-run in
  ...
  `c3ffe169`** â€” the import is deterministic; no restore needed (pre-import
  `14f12fab` stayed recoverable from monorepo HEAD throughout).
- **Gate (import_gate_verify, fresh both runs): ALL GATES PASS** â€” all 19
  ...
  (+114 raw / +76 rolled / +28 active; ambestâ†’serff flip, 0 interim remnants);
  as-of pin held (2026-06-11).
- **B9 INTERIM CORRECTION EXACT:** ambest âˆ’192 raw / âˆ’158 rolled = ND's
  replaced interim block (115/81) + exactly the 86 mislabeled rows (77 across
  AR âˆ’2, IN âˆ’16, KS âˆ’6, MN âˆ’3, NE âˆ’7, NJ âˆ’2, OK âˆ’13, PA âˆ’8, SD âˆ’8, TN âˆ’2,
  WI âˆ’10, + ND's own 9). Every state's delta == its pattern-row count; **0
  ...
  over-match after Wausau â€” bare-substring place/occupation names are a
...
- **CROSS-CHECK (10th interimâ†’real point):** value-agreement on shared 44/44
  (100%), interim 45/45 (0 disagreements), in-window PPA 35/35 + HO 20/20 =
  100%. **Coverage verdict RICHNESS-ONLY on all 4 divergence brands â†’ the
  coverage-thinness pattern REFINED: it is a VERY-ISOLATED-market phenomenon
  (AK), NOT general small/rural â€” ND is small/rural but integrated and
  AM-Best-complete.**
- ND dropped from `AMBEST_STATES` (â†’ 25), KEPT in `COVERED_STATES` (the NH
  trap); `validated:{auto:true, home:true}`; methodology 19â†’20 rows, ND âœ“âœ“.
  verify_subtype re-keyed 566â†’594 (per-sub_type deltas exactly ND's: PPA +10,
...
- **HEADs (post-deploy):** insurancewebscraper `ef59e58`->**`9645a69`**
  (HEAD==origin; all_states 3092â†’3206, 0 deletions; ND collection/cross-check/
  ...
  0 deletions; Vercel LIVE â€” /,/methodology,/prospect,/defend 200, ND renders,
...
# Session checkpoint â€” 2026-07-06 (MA)  Â·  MA INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (19th scraped state; first medium tier)

MA's 129-row SERFF scrape (281/282 in-target, 0 true misses) replaced its AM
Best interim; cross-checked (9th point â€” VALUE-AGREEMENT 53/53 = 100%, INTERIM
...
- **Gate:** 18 prior scraped states byte-identical (incl. B6-corrected
  ...
  enriched** (TravCo/Phoenix â†’ Travelers brand-map fix: MI/ND/SD/PA/RI/TN,
  +13 raw/+6 rolled â€” the WV/NH globally-correct precedent; NJ/WI reverted to
  byte-identical after the Wausau exclusion).
- **COVERAGE VERDICT: richness-only** â€” GEICO/Progressive/Farmers AM-Best-0
...
- **Exclusion doctrine resolved:** LM P&C + Wausau family excluded (retired-
  brand/no-consumer-channel LM vehicles, Peerless-consistent â€” Peerless was
...
- **NICOA paginator-glitch save end-to-end:** +6.1%/4,038ph HO renders live,
  ...
  AMBEST_STATES â†’ 26; methodology 19 rows, MA âœ“âœ“.
- **HEADs (post-deploy):** insurancewebscraper `09cdf15`->**`09a4305`**
  (HEAD==origin). agent-intel/master `8b5f858`->**`81fba2b`** (subtree-FF, 0
  deletions; Vercel LIVE â€” MA Prospect serves incl. the NICOA +6.1% signal).
...
# Session checkpoint â€” 2026-07-07  Â·  B6 COMPLETE: comma-percent parser fix + 18-state retro-verification + 7-state gated re-import

The "wrapped-name" backlog bug root-caused to the shared parser's row regexes
rejecting thousands-comma percentages ("1,830.500%") â†’ silent 0-row / PARTIAL /
...
- **THE RECOVERED SIGNAL: IL CFPC-134419708 â€” a LIVE Defend row** (COUNTRY
  Financial âˆ’4.38% rolled / 334,163 ph / eff 09/21/2025, Personal Auto). Both
...
- **Also corrected:** WV FAIG-134672434 wrong-value 0.000%â†’2.800% (old parser
  fell through to a stale pre-amendment row â€” adjudicated at source PDF); GA
  rollups â†’ source-true (CFPC-133859077 16.98â†’17.56%, CFPC-134294144 3.376â†’3.467%
  ...
  (AK native parse == recover_ak_cfpc adjudication exactly â€” no-op).
- **GATE (import_gate_verify capture-diff): 38 states byte-identical** incl.
  ...
  verify_subtype 551 (Auto-Comb 81â†’82 == exactly the IL filing); 8 verify + tsc
...
- **LESSONS (recorded in scraper resume_state.md):** (1) a symptom sweep catches
  ...
  corruption â€” the diff found 6 cases the sweep couldn't. (2) GA/AZ/NM
  deliverables carry post-build AM-Best tier enrichment a raw re-finalize
  silently downgrades (314 rows) â†’ surgical append-only for enriched states.
- **HEADs (post-deploy):** insurancewebscraper `97a7edf`->**`a439e2b`** (HEAD==
  ...
  Vercel LIVE â€” the IL COUNTRY Defend row confirmed serving from the production
...
# Session checkpoint â€” 2026-07-06  Â·  COPY/CLARITY CHANGE SET SHIPPED (6 items â€” UI copy + spec + e2e)

Copy-only iteration (no data, query, or routing changes; `filings.db` untouched â€”
...
- **Overview title** â†’ **"Overview of Rate Change Activity"** (`page.tsx` main
  h1 + the error-state h1; nav label stays "Overview").
- **Prospect/Defend subtitles per agent type:** captive *"Rate increases/decreases
  filed by {brand}'s competitors in your statesâ€¦"* / independent *"â€¦in your states
  from competitors and carriers you sellâ€¦"*. Captive P/D excludes the own brand so
  "competitors" is literally true; independent includes sold carriers so it can't
  say that â€” the split is the point, don't unify it.
- **Positioning date-range note** (new, under the subtitle, testid
  ...
  {Mon D, YYYY}."* â€” derived dynamically from `asOf`
  (`src/lib/positioningExplainer.ts` `dateRangeNote`). **DELIBERATELY 12-month
  wording, NOT "since 2024"** â€” recon proved Positioning applies the rolling
...
- **Plain-language explainer** (new `buildExplainer` + "How to read this" box on
  /positioning): interprets ONE real comparison from the agent's own view â€” the
  richest cell's largest-|spread| **high-confidence** comparison.
  **Both-averages-plus-spread template** â€” the differential is the spread
  (compAvg âˆ’ agentAvg), NEVER a side's own average; keeps the word "filed";
...
- **Tab rename** â†’ **"Competitive Positioning"** (`NavBar.tsx`; never "Pricing" â€”
...
- **spec.md updated alongside the code (the conflict rule):** copy table, Overview
  ...
  re-keyed: `e2e_setup` Overview heading; `e2e_nav` label arrays + routeâ†”label
...
- **Gates:** tsc clean, 8 verify scripts, prod build 12/12, 13/13 e2e
  ...
  PASSED" â€” don't grep it as a failure). Localhost-approved.
...
# Session checkpoint â€” 2026-07-05  Â·  AK INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (18th scraped state)
...
home:true}`. Deterministic rebuild of `filings.db` (md5 `d3075558` â†’ `23f0bf29`);
deployed via subtree-FF to **agent-intel/master** (Vercel LIVE â€” AK renders
directly-scraped, methodology 18). AK is a COVERAGE-THIN AM Best state â€” the 8th
interimâ†’real point, and the first where AM Best undercounts a SPECIFIC major writer.
...
- **âš ï¸ COVERAGE-THIN finding (CORRECTED):** AM Best undercounts **COUNTRY
  specifically** (32 real in-target vs 2 shown; 6 rolled rate filings vs 2) â€” the
  ...
  NOT a gap** â€” their AK in-target-by-line filings are ALL Form/Rule (no rates), so
...
- **MATERIAL RECOVERY:** 2 COUNTRY CFPC (wrapped-name parser bug, VT PRGS family
  2nd occurrence â€” now MATERIAL) recovered via `recover_ak_cfpc.py` (targeted,
  AK-only, asserted vs adjudication). B6 backlog: shared-parser fix (MEDIUM).
- AK dropped from `AMBEST_STATES` (â†’ 27), KEPT in `COVERED_STATES` (NH trap);
  methodology 17â†’18. Covered total unchanged (45 = 18 scraped + 27 AM Best).
...
# Session checkpoint â€” 2026-07-01  Â·  HI INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (17th scraped state)
...
home:true}`. Deterministic rebuild of `filings.db` (md5 `be5417b7` â†’ `d3075558`);
deployed via subtree-FF to **agent-intel/master** (Vercel LIVE â€” HI renders
...
- **EVERY delta == HI:** raw +71, rolled +35, active +17, states +1. **16 prior
  ...
  the new **`import_gate_verify.py`** â€” a REUSABLE interimâ†’real import gate
  (generalized from HI; every future import runs it): ALL GATES PASS.
- HI dropped from `AMBEST_STATES` (â†’ 28 states), KEPT in `COVERED_STATES` (the NH
  trap); as-of PINNED 2026-06-11 (xlsx mtime restored â†’ window didn't slide);
  methodology derived 16â†’17. Covered total unchanged (45 = 17 scraped + 28 AM Best).
...
- **CROSS-CHECK (7th interimâ†’real point, CLEANEST YET â€” `compare_hi_ambest.py`):**
  ...
  agreement, 0 disagreements** â€” no AM-Best-side dispute (no SFMA-style case).
  ...
  PPA leg 44/44 â†’ `validated:home` sound. **AmFam pre-resolved = 0 genuine**
  (AM Best HI 0 in-scope AmFam; our 0 AmFam-brand = all excluded subs â€” Munich Re
  Home + Connect). **VA 97.7 / OH 100 / IL 99.4 / WV clean / NH 100 / VT 100 /
  HI 100 â€” the AM Best interim is sound 7Ã—.**
- **DELIVERABLE `hi_final_rates.xlsx` = 71 rows** (PPA 59 / HO 12; funnel 138 =
  42 emitted (79 âˆ’ 8 filing-vehicle) + 85 form/rule + 4 new-product + 7 out-of-window).
- **Scraper-side banked:** insurancewebscraper `e93d5ea`â†’`f27785c` (compare +
  cross-check checkpoint) â†’ `55c9268` (all_states regen + import-gate tool).

---

# Session checkpoint â€” 2026-06-30  Â·  VT INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (16th scraped state)
...
rebuild of `filings.db` (md5 `5c821d8b` â†’ `be5417b7`); deployed via subtree-FF to
**agent-intel/master `74b6082`** (Vercel LIVE â€” /,/methodology,/prospect 200,
"15 of 16 cross-checked states", Vermont âœ“âœ“).
...
- **FIRST FULLY-HARDENED-PIPELINE STATE.** VT is the payoff of the root-pattern
  ...
  already carried those terms â†’ clean collection â†’ clean cross-check â†’ **0
  ...
  gap-class entity).
- **CROSS-CHECK (6th interimâ†’real point, CLEANEST YET):** corroboration PPA 26/32
  (81.2%) / HO 16/17 (94.1%); the **7 AM-Best-only rows are ALL recency or
  immaterial** (3 recency eff > 04/17/26 window: Farmers Ã—2 04/20/26, Allstate HO
  05/21/26; 4 zero-impact: the USAA 02/21/26 0%/0ph cluster) â†’ **material in-window
  ...
  NH 100 / VT 100 â€” the AM Best interim is sound 6Ã—.** Richer: 54 back-extension +
...
- **NEW SCRAPED BASELINE (SUPERSEDES post-NH `2673`/`1673`/495 / `5c821d8b`):**
  **16 states**, `filings_raw` serff_scraped **2776**, `filings` **1738**,
  active@2026-06-11 **510**, db md5 **`be5417b7`**, anchor +93.70% WA (unchanged â€”
...
- **EVERY delta == VT:** raw +103, rolled +65, active +15, states +1. **15 prior
  ...
  VT dropped from `AMBEST_STATES` (â†’ 29 AM Best states; kept in `COVERED_STATES` â€”
  the NH near-miss trap, verified VT stays covered). Methodology **16 rows, VT âœ“âœ“**.
- **GATES:** import 16/16, verify_subtype (510) + 7 verify, tsc clean, build 12/12,
  e2e 13/13 (fixed the one stale `15 rows`â†’`16 rows` methodology assertion).
  Localhost-reviewed + approved. Scraper-side: `compare_vt_ambest.py` + all_states
  regen â†’ insurancewebscraper `6b8a7d1`. Monorepo `de4e003` (local).
- **âš ï¸ OPERATIONAL FLAGS carried:** (1) **browser-launch-kill** â€” the env kills
  ...
  at launch. (2) **wrapped-company-name parser bug** (`no_pdf==1`, PRGS-134029613) â€”
  ...
  states byte-identical) â€” backlogged, no urgency. (3) harvest-early
  tail-over-stop (NH b4, VT b2 â€” 3rd occurrence, minor tuning backlog).

---

# Session checkpoint â€” 2026-06-29  Â·  NH INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (15th scraped state)

NH's 113-row SERFF scrape (214/214 in-target) replaced its AM Best interim;
cross-checked FIRST, shipped `validated:{auto:true, home:true}`. Deterministic
rebuild of `filings.db` (md5 `1147ba06` â†’ `5c821d8b`); deployed via subtree-FF to
**agent-intel/master `cd796bd`** (Vercel LIVE).

- **CROSS-CHECK (5th interimâ†’real point, CLEANEST YET):** in-window PPA **22/22
  ...
  RECENCY** filings (May-2026 effective, beyond the scrape window â€” incl. big-ph
  Progressive Universal 107k / State Farm Fire 86k) â€” coverage/recency, not errors.
  **VA 97.7 / OH 100 / IL 99.4 / WV clean / NH 100** â€” the AM Best interim is sound
  5Ã—. 0 blank max/min (clean parser; no SF `% %`, no minus-family bug).
- **2nd SYSTEMATIC SEARCH-TERM GAP FIXED GLOBALLY (after Liberty):** the lone genuine
  ...
  +15.8%, **3,711ph** â€” a Prospect signal). A subsidiary whose legal name lacks the
  brand string â€” the **ROOT PATTERN** shared with Liberty Insurance Corporation.
...
- **GATES:** import 16/16, verify_subtype (495) + 7 verify, tsc, build 12/12, e2e
  ...
  config fix + all_states â†’ insurancewebscraper `7261209`. Monorepo `1e14362`.
- **âš ï¸ BACKLOG (root-pattern gap class â€” 2 systematic gaps found so far):**
  (1) retro-audit shipped states (VA/OH/IL/WV) for missed "General Insurance" /
  "Liberty Insurance" filings with the fixed terms â€” recover MATERIAL ones (a small
  ...
  Insurance Corp + General Insurance Co of America belong to) â€” close the whole gap
  class before scraping more states.

---

# Session checkpoint â€” 2026-06-26  Â·  WV INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (14th scraped state)

WV's 113-row SERFF scrape (207/207 collected) replaced its AM Best interim;
cross-checked FIRST, shipped `validated:{auto:true, home:true}`. Deterministic
rebuild of `filings.db` (md5 `19a94a1f` â†’ `1147ba06`); deployed via subtree-FF
...
- **RANKING-FIRST:** ranked the remaining ~29 AM Best interim states offline by
  ...
  smallest clean candidate â†’ harvested smallest-first. AM Best in-target RANKS
  reliably even though absolute scrape size differs (WV 43â†’113, ~ the VA/OH/IL
  proxy held the ordering).
- **CROSS-CHECK (4th interimâ†’real point):** PPA **30/33 (90.9%)**, HO **9/10
  (90%, lone miss pure recency â†’ 9/9 in-window)**, interim **37/40** â€” the 3
  ...
  VA 97.7 / OH 100 / IL 99.4 / WV** â€” the AM Best interim is sound 4Ã—, strong
...
- **MATERIAL SOFT-MISS RECOVERED (new precedent):** the cross-check surfaced 1
  MATERIAL AM-Best-only â€” Liberty Insurance Corporation HO eff 01/09/25 âˆ’5% /
  **7,894ph** (a real Defend signal). Root cause: a search-term gap (the
  Allstate/Encompass family) â€” "Liberty Insurance Corporation" has no "mutual".
...
- **WV dropped from `AMBEST_STATES`** (â†’ 31 AM Best; CA/NY/TX permanent intact).
  Methodology â†’ **14 cross-checked rows, WV âœ“âœ“**.
- **GATES:** import verify 16/16, verify_subtype (480) + 7 verify scripts, tsc
  ...
  `compare_wv_ambest.py` + `all_states` regen â†’ insurancewebscraper `e3c8bf9`.
  Monorepo `d4adf7d` (local, origin-behind norm).

---

# Session checkpoint â€” 2026-06-25 (cont.)  Â·  IL INTERIMâ†’REAL IMPORT SHIPPED + VALIDATED (13th scraped state)
...
deterministic rebuild of `filings.db` (md5 `3e7d83fe` â†’ `19a94a1f`); deployed via
subtree split to **agent-intel/master**.

- **CROSS-CHECK (offline, before import):** PPA **93.8%** (122/130), HO **95.2%**
  (60/63), interim agreement **155/156 (99.4%)** â€” the lone differ a 0.1% rounding
  diff. **3rd interimâ†’real data point: VA 97.7% / OH 100% / IL 99.4% â€” the AM Best
  interim is confirmed sound 3Ã—**, strengthening confidence the ~29 remaining
...
- **NEW SCRAPED BASELINE (post-IL â€” SUPERSEDES post-OH `2144`/`1330`/392):**
  **13 states**, `filings_raw` serff_scraped **2447 rows `26a01779`**, `filings`
  **1536 rows `2510c9da`**, active@2026-06-11 **461**, db md5 **`19a94a1f`**,
  anchor +93.70% WA (unchanged â€” IL max below it).
- **EVERY delta == IL, nothing more:** raw 2144â†’2447 (+303), rolled 1330â†’1536
  (+206), active 392â†’461 (+69). IL `ambest_sourced` remnants **0/0**.
- **BYTE-IDENTICAL PROOF:** the **44 non-IL states** identical in BOTH tables
  (count + content hash), incl. **VA `be861571` + OH `5c9ba09e`** (prior
  interimâ†’real) and **CA/NY/TX permanent**. As-of pinned 2026-06-11.
- **LBPM-134662340 OPTION-A RESOLUTION:** IL's one multi-brand co-filing (PPA,
  ...
  (Group filer; Montgomeryâ†’Liberty precedent). Scoped to the EXACT
  `{Liberty Mutual, Safeco}` span â€” every other multi-brand span still FATALs;
...
- **UNIVERSE SOFT-MISSES (tracked, immaterial):** the Liberty HO (eff 04/25/25,
  5.9%) AM Best-only row + the Truck-Ins-Exch first-sweep clean-0 â€” both single
  ...
  individual filing.
- **IL dropped from `AMBEST_STATES`** (â†’ 32 AM Best states; CA/NY/TX permanent
  intact). Methodology data-driven â†’ **13 cross-checked rows, IL âœ“âœ“**.
- **GATES:** import verify 16/16, verify_subtype (active 461) + 7 verify scripts,
  ...
  Scraper-side: `all_states_final_rates.xlsx` 2144â†’2447, `compare_il_ambest.py`
  â†’ insurancewebscraper.

---

# Session checkpoint â€” 2026-06-25  Â·  OH AM Best CROSS-CHECK DONE â†’ OH VALIDATED (closes the open follow-up)

Offline cross-check (`compare_oh_ambest.py`, adapted from `compare_va_ambest.py`;
no SERFF, **`filings.db` NOT mutated** â€” md5 still `3e7d83fe`). OH's
`validated:{false,false}` â†’ **`{auto:true, home:true}`** (states.ts + e2e). The
open follow-up from the import session is now CLOSED.

- **(a) STANDARD â€” scrape values correct:** PPA **74/77 = 96.1%**, HO **34/35 =
  97.1%** corroboration (combined 108/112 = 96.4%). **Beats VA (92.3% PPA) & GA
  (93.8%)**; OH HO especially strong vs VA's small-N 68.6%.
- **(b) INTERIM-QUALITY â€” 105/105 (entity,eff) impact agreement, 0 disagreements**
  (VA was 97.7%/127-of-130). **2nd interimâ†’real data point, even cleaner than VA**
  â†’ strong evidence the AM Best interim is sound across the remaining **33** AM
  Best states. Zero disagreements â†’ no SFMA-style "AM Best is wrong" case.
- **(c) REVERSE â€” scrape richer:** 57 rate-neutral 0% filings + 128 pre-2025
  ...
  immaterial 0% coverage: SF Fire/Mutual 04/15/25, Liberty 05/19/25) â€” coverage/
  recency, **not value errors** (the VA pattern).
- **WATCH cleared:** 6 blank-max/min rows = 3 State Farm co-filed pairs with
  POPULATED impacts (documented SF `% %` source pattern, 3Ã— precedent); none
  coincide with the 4 AM Best-only â†’ no parser drop. No parser-family-bug signature.
- **Methodology now 10 of 12 cross-checked** (OH joins; CO still the lone scraped-
  but-unvalidated, "â€” â€”"). Gates: tsc clean, build 12/12, e2e_methodology + 13 e2e
  + verify_subtype + 7 verify scripts green. `compare_oh_ambest.py` â†’ insurancewebscraper.

---

# Session checkpoint â€” 2026-06-24 (cont.)  Â·  OH INTERIMâ†’REAL IMPORT SHIPPED (12th scraped state)
...
surgical pattern as the VA import. **filings.db rebuilt** (md5 `a1c5cda4` â†’
`3e7d83fe`); deployed via subtree split to **agent-intel/master**.

**Shipped HEADs (all HEAD==origin):** agent-intel/master **`c3e92da`** (Vercel
LIVE â€” root/methodology/prospect HTTP 200, methodology renders "Ohio Â· directly
scraped") Â· insurancewebscraper **`49bdacd`** Â· monorepo **`83580c5`** (local,
origin-behind norm). Deploy was a clean FF onto agent-intel/master (subtree
split produced content-identical trees w/ divergent hashes â†’ committed the new
tree onto the real 4372224, no force-push, 0 file deletions).

### â–¶ OPEN FOLLOW-UP (next session) â€” OH AM Best cross-check
OH currently ships **`validated:{false,false}`** â€” HONEST: directly-scraped real
data, NOT yet AM Best cross-checked (renders "â€” â€”" in methodology, like CO).
**Pick up here:**
1. Adapt `compare_va_ambest.py` â†’ `compare_oh_ambest.py` (the GA/VA 13-brand
...
3. **If it corroborates â†’ flip OH to `validated:{true,true}`** in a small commit:
   2 booleans in `states.ts` + the `OH` line in `e2e_methodology.ts` EXPECTED map
   (`{false,false}`â†’`{true,true}`, "â€”"â†’"âœ“") + methodology count auto-updates.
...
4. **WATCH (VA precedent):** the AM-Best-parser-family bugs AND the
   ...
   truth â€” investigate disagreements at the source PDF before assuming a scrape error.
...
- **Backlog (none urgent):** AL/FL/LA AM Best re-pull (header-only, fixable) Â·
  NC structural NCRB gap (not fixable â€” needs a different source) Â· 3 harvest
  tooling fixes (`BACKLOG.md`: harvest-early ordering re-enable, per-group
  not-found stop, **B5** front-of-batch grace) Â· monorepo `origin` ~96 behind
  (established norm; deploy path is agent-intel, not origin).

- **NEW SCRAPED BASELINE (post-OH â€” the intended new normal; SUPERSEDES the
  post-VA tags `filings_raw a1aa7a2e`/1893 Â· `filings ea8ba81e`/1176 Â· 342):**
...
- **EVERY delta == VA + OH, nothing more:** raw 1893â†’2144 (VA 277â†’282 = +5, OH
  +246); rolled 1176â†’1330 (VA 178â†’182 = +4, OH +150); active 342â†’392 (VA 49â†’51 =
...
- **BYTE-IDENTICAL PROOF:** the **43 non-VA/OH states** identical in BOTH tables
  ...
  (all_states xlsx mtime) so the 12-mo window didn't slide â†’ western active
  counts identical. **VA `validated:{auto,home}` true; OH `validated:{false,
  false}`** â€” OH is directly-scraped REAL data but NOT YET AM Best cross-checked
...
- **OH dropped from `AMBEST_STATES`** (import_filings.py + constants.ts);
  ...
  data-driven â†’ **12 cross-checked-table rows** (OH/CO show "â€” â€”").
...
- **VA collection delta:** the +5-row VA refresh (282 vs the shipped 277) is now
  live too. Scraper-side `all_states_final_rates.xlsx` regenerated 1893â†’2144 â†’
  insurancewebscraper. Full scraper history: `resume_state.md`.

---

# Session checkpoint â€” 2026-06-24  Â·  OH COLLECTION COMPLETE (scraper-side); import pending

SERFF session â€” OH download harvest resumed from 45% and **finished to 100%** in
8 cold bursts. **filings.db / the app were NOT touched this session** â€” pure
...
- **Universe resolved:** the "361 distinct" estimate â†’ **397 actual target
...
- **WAF held all session:** full-burst 405-challenge series **40â†’36â†’30â†’42â†’20%**;
  the 42% (burst 7) was a heavy-load tail (walls all post-completion), **not**
  cumulative depression â€” no bank-signal ever materialized. The long cold rest
...
- **Q2 RECOVERY proven in vivo** (first time since VA): RECOVERY-1 landed all 4
  ...
  was transient and recovered â€” hence 0 true misses.
- **BACKLOG B5 recorded** (`Insurance Rate Data Scraper/BACKLOG.md`):
  front-of-batch grace â€” the 2-consecutive-miss early-stop can abort a whole
...
- **IMPORT PENDING:** combined **VA-refresh (+5 rows)** + **OH import (246-row
  scrape)** into `filings.db` â€” the next deliberate app-side step (surgical
...
# Session checkpoint â€” 2026-06-23  Â·  VA COLLECTION COMPLETE (scraper-side); OH not started

SERFF session â€” first contact since the 06-22 VA harvest. **VA scraping is now
effectively complete**; OH recon is the next step (not yet started this checkpoint).
**filings.db / the app were NOT touched this session** â€” pure scraper-side work.
...
- **VA COLLECTION COMPLETE: 444/450 in-target cached (98.7%)**, `va_final_rates.xlsx`
  ...
  burst-1 (warm-tail) +17 â†’ 436/450, burst-2 (clean cold) +8 â†’ 444/450.
  ...
  pre-commit â€” restored 8 purged tracked files from HEAD, then explicit-path staged).
- **`TRVD-G` "coverage-gap" HYPOTHESIS DISPROVEN.** The G-series is **normal,
  downloadable Travelers data that surfaces under bare `travelers`** â€” NOT a hidden
  sub-entity and NOT a new-search-term gap. The apparent gap was (a) download-
  completion (WAF-interrupted empty dirs) + (b) a **tracking#â‰ filingId keying
  artifact** (e.g. `TRVD-G134180979` â†’ filingId `134191017`; PDFs cache by filingId).
  The fallback recovered G-series directly once WAF allowed. This is the 4th VA
  "limit" to turn out an artifact â€” consistent with the whole VA arc (row-wall,
...
- **6 genuinely uncollectable** (survived TWO full attempts incl. a clean cold burst):
  ...
  (fid 134883989) â€” all Quantum Home/Auto. Accepted as uncollectable-under-`travelers`;
...
- **WAF:** two bursts spent today; **ceiling held** (no slow-penalty depression â€”
  consistent with yesterday's heavier single-day load). Cold refill behaved as modeled.
- Full scraper history: `Insurance Rate Data Scraper/output/resume_state.md`.

---

# Session checkpoint â€” 2026-06-22  Â·  VA interimâ†’real import SHIPPED (11th scraped state)
...
since the 45-state load. **Deployed (agent-intel/master): `8b813c0`** â€” live
...
- **NEW SCRAPED BASELINE (post-VA â€” the intended new normal; SUPERSEDES the old
  ...
  **1176 rows `ea8ba81e`**, active@2026-06-11 **342**. The 10 original scraped
  states + CA/NY/TX permanent stayed **byte-identical** (rows AND active counts â€”
...
- **VA**: now `source=serff_scraped` (178 rolled), 0 `AMB-` surrogate keys, richer
  ...
  â†’ Liberty Mutual (VA LBPM co-filer; VA-only, 10 states unaffected). AM Best
...
- **Scraper-side** (insurancewebscraper `b1ffb28`): `all_states_final_rates.xlsx`
  regenerated 1616â†’1893 (VA appended last). VA collection itself ~91% (452/498) â€”
...
# Session checkpoint â€” 2026-06-19  Â·  Two UI/profile iterations shipped (45-state data)

Iterate-and-deploy mode, not building. **Two iterations shipped this session**,
both UI/profile-data only â€” the scraped baseline stayed byte-identical and
`filings.db` was never in any changeset.

- **Deployed (agent-intel/master): `546d3e4`** (office addresses â€” latest of the
  two). HEAD==origin, Vercel auto-deploying. Monorepo local `8545992`
  (`origin/master` `fc11c86`, ~96 behind â€” established norm; deploy path is
...
- **Quiet period ACTIVE until 2026-06-21** â€” fully offline session, zero
  SERFF/WAF/guard contact. The June-21 cold-capacity read is still the next
  pending scraper action (carry-forward below).

## Iteration 1 â€” Recent Changes rows: two-block flex layout (dashboard, `554d90a`)
...
- **Left block** (flex:1): impact % (62px, right-aligned, category-colored â€”
  Prospect green / Defend red) sitting **directly next to** carrier Â· line Â·
  state, with **"date Â· N policyholders"** as a muted secondary line beneath
  (policyholders folded back inline â€” no standalone column; count omitted on AM
...
- Defend/Prospect tables left untouched (real tables with their own
  Policyholders/Effective columns â€” they read fine).
- **LESSON (recorded for future spacing work):** this content is light for a
  full-width container â€” **group into tight blocks, don't spread across the
  width**. Took 4 iterations to land. Start future Recent-Changes-style spacing
  from "group tight," not "spread wide."

## Iteration 2 â€” Office addresses replace standalone home state / ZIP (Agency Profile, `546d3e4`)
...
- **Form:** each office = optional label, street, city, state, ZIP. One office by
  default ("Primary office"); **+ Add another office** appends, **Ã—** removes;
  **can't drop below one** (Ã— hidden when a single office remains). No standalone
...
- **Data model:** `AgentProfile` **dropped `home_state`/`zip_code`, gained
  ...
  `primaryOffice(p)` accessor. **Validation:** â‰¥1 office; street/city/state/ZIP
...
- **Migration (no data loss):** `loadProfile` runs `migrateProfile` in place â€” a
...
- **`home_state` HAD downstream consumers** (it drove the **/compliance briefing
  ...
  `ComplianceBriefing`) take a state-**string param** â€” unchanged, just fed from
  the primary office now. **`zip_code` had no logic consumers.** tsc confirms no
  orphaned reads (both fields are gone from the type).
- **FUTURE NOTE:** office addresses are **recorded profile data only** â€” they
...
## âš ï¸ Recurring gremlin â€” stale "CA non-covered" test assertion

Fixed **twice** now (`e2e_setup`, `verify_profile`, both CAâ†’WY). CA/NY/TX became
**permanent covered** states in the 45-state expansion. **If a 3rd surfaces,
sweep ALL tests at once** for hardcoded CA/NY/TX non-covered assumptions â€” the
...
- **June-21 cold-capacity read = next pending decision.** Clear the guard â†’
  `cold_capacity_read.py` **BEFORE any collection** (collecting first
  contaminates it). Low pressure â€” the app already has near-national coverage.
  Parked scrapes: VA 165/498 PDFs, OH partial.
- **Re-pull list** (header-only exports, fixable): AL/FL/LA. **Structural gap:**
  NC (rates via NCRB collectively â€” not fixable via AM Best). **WY:** no rows.
- Monorepo `origin/master` ~96 behind (`fc11c86`) by established norm; deploy
  path is agent-intel/master. Left as norm (not synced).

---

# Session checkpoint â€” 2026-06-18  Â·  UI polish pass (tables + dashboard) on 45-state data
...
- **Deployed (agent-intel/master): `95534d5`** (UI polish pass, UI-only).
  Monorepo `755a357` (local; `origin/master` `fc11c86`, ~96 commits behind â€” see
  ...
  (`filings_raw 784f77e6`, `filings b6f83d78`; `md5(filings.db)=82e423fdâ€¦`).

## Latest iteration â€” UI polish pass: tables + dashboard (2026-06-18, UI-only)

A presentation-only restyle of the Prospect/Defend tables and the Overview
dashboard. **No data, query, filtering, or routing changes** â€” same rows, same
counts, purely how they read. Shipped `95534d5` after localhost review of all
three surfaces.

- **COLOR SYSTEM â€” agent-perspective (the organizing idea).** Everything is colored
  by what a move MEANS FOR THE AGENT, not by raw rate direction: **Prospect = green**
  (competitor raised â†’ your opportunity), **Defend = red** (competitor cut â†’ your
  ...
  retention-risk red, own decrease = opportunity green) â€” already agent-perspective.
...
- **Tables (shared `FilingsTable`, all three modes).** State folded into the carrier
  cell ("Allstate Â· ID"); **sub-type + its info bubble moved to a muted secondary
  line** (`SubtypeCell` now inline) â€” State and Sub-type columns dropped, which frees
  ...
  per-row callout was removed (the page header states it once) â†’ defend in-effect
  badge is now the factual **"In effect N weeks"** (`computeWindowBadge`); Approved
  status color changed blueâ†’green in `computeStatusBadge`.
- **Dashboard.** Four cards are flex-columns with **bottom-aligned "View all" links**
  ...
  **Recent Changes rebuilt as a fixed-layout `<table>`** (was a flex list that
  clustered at the edges): leading category-colored impact, `carrier Â· line Â· state`
  with the **effective date anchored as a muted line beneath it** (a standalone date
  column read as orphaned â€” reverted to date-under-carrier), a right-aligned
  **Policyholders column** (count over a muted label, graceful **"â€”" when missing** so
  ...
  left as-is â€” their date is already anchored in the Effective column (date stacked
...
- **Tests.** e2e updated for the new column layout (`row-brand`/`row-state` testids,
  ...
  the feed). **`verify_profile.ts` CAâ†’WY stale-test fix** â€” its tamper case asserted
  "CA is non-covered", but **CA became a permanent covered state** in the 45-state
  expansion. âš ï¸ **This is the 2nd place this exact stale "CA non-covered" assertion
  surfaced** (the first was `e2e_setup`, fixed earlier). **Pattern flag:** if a 3rd
  appears, sweep all tests for hardcoded "CA"/non-covered-state assumptions at once â€”
  the covered-state set grew and any test still treating CA (or NY/TX) as non-covered
  is now stale.
- **Deploy hygiene â€” monorepo-origin drift (recorded, left as the norm).** The
  ...
  per-session sync. **Not synced this session** â€” left as the established norm. Push
...
- **Build/dev gotcha (learned this session):** running `npm run build` while
  `npm run dev` is live clobbers the dev server's `.next` chunks â†’ `MODULE_NOT_FOUND`
  500s on every route. Fix: stop dev â†’ build â†’ `rm -rf .next` â†’ restart dev. Don't
  run a prod build against a running dev server.

## Latest iteration â€” Own-carrier alerts: retention + opportunity (2026-06-17, UI-only)
...
- **`src/lib/retention.ts` (new, shared single source):** `computeRetentionRisk`
  ...
  `effective_date`, anchored to data as-of â€” tighter than the 12-month
...
- **My Carriers page:** band gained **Retention risk** (red) + **Opportunity**
  ...
  recent own-carrier increases. Plain-word wording ("5% or more, last 6 months") â€”
  no math symbols. (5-stat band fits one row at desktop, reflows 3+2 narrow.)
- **Dashboard:** **"Most Urgent" REMOVED** (`computeMostUrgent` dropped) â†’ replaced
  by a two-direction **"My Carrier" alert card** (retention + opportunity counts,
  both â†’ /my-carriers). **"Your Carrier Snapshot" table REMOVED** (`CarrierActivity.tsx`
  deleted, `computeCarrierActivity` dropped). Recent Changes unchanged.
- **Stale test fixed:** `e2e_setup` CAâ†’WY â€” CA became covered/permanent in the
...
- (a) own-carrier-increase-as-Defend was deliberately NOT folded into competitive
  Defend â€” lenses kept distinct (resolved by the My Carrier card).
...
## Latest iteration â€” AM Best 22-state batch (2026-06-17)
...
- **998 scraped** (13-brand: AZ CO GA ID MT NM NV OR UT WA) â€” untouched.
- **AM Best (35 states):** 32 INTERIM (replaceable when scraped) + 3 PERMANENT.
  ...
  - **PERMANENT (3): CA, NY, TX** â€” non-SERFF (CDI/DFS/TDI), never replaceable.
    ...
    tag: when unsure, prefer permanent** (mis-tag scrapeableâ†’permanent = "revisit
    manually"; mis-tag non-scrapeableâ†’interim = replacement sweep deletes with no
...
NJ + NY are **0-Defend, confirmed real** (active windows all 0%â†’positive, nothing
mis-classified â€” DOBI/DFS prior-approval regimes). B2: prior-approval NY/NJ/MA
show `eff>disp` dominant (correct); MD/TX `eff<disp` = normal file-and-use, not
a swap.

**DOCUMENTED GAPS (not covered, 5 states):**
- **AL, FL, LA â€” re-pull list (FIXABLE).** Header-only exports (no disposition
  data). Re-export from AM Best WITH the disposition supplement. (FL also non-SERFF.)
- **NC â€” STRUCTURAL, NOT fixable via this source, NOT on the re-pull list.** NC
  rates auto/home through the NC Rate Bureau (NCRB) collectively â†’ AM Best has no
  per-carrier data (68.1% Data-N/A â†’ 26 thin filings). A re-pull will NOT help;
  needs a different source (NCRB direct / another vendor) if ever wanted.
- **WY â€” no rows** (listed, no filings).

## Latest iteration â€” AM Best 10-state batch (2026-06-17)
...
- **AM Best INTERIM (12, replaceable when scraped):** IL, OH, VA + IN, KY, IA,
  AR, CT, KS, DE, AK, HI.
- **AM Best PERMANENT (1, NOT replaceable):** **CA** â€” non-SERFF (CDI runs its
...
(no column swap). **CA 0-Defend is real** â€” its active window is 0%â†’positive only
(no approved decreases; Prop-103 regime), not a sign/classification artifact.

**RE-PULL LIST (excluded from this batch â€” header-only exports, a PULL problem):**
- **AL** (100% "Disposition Page Data N/A"), **FL** (100% N/A, and ALSO non-SERFF
  like CA), **LA** (86.8% N/A â€” only a 14-filing sliver was usable).
- These exports lack the disposition supplement â†’ no rate-effect % (the core
...
- **Quality gate for any future AM Best batch:** check the `Disposition Page
  Data N/A` rate before loading. Loadable states sat at 1.9â€“43.4% (normal);
  ~100% = unusable header-only pull.

Deploy this batch went agent_intelligence â†’ agent-intel/master via a **git
...
## Latest iteration â€” AM Best interim data for IL/OH/VA (2026-06-16)
...
states â€” NO UI marker**. The distinction is **backend-only**:
...
- Backend-only surrogate key `AMB-<ST>-<line>-md5(block_id|brand)` (block-stable,
  idempotent, collision-proof) â€” **stripped from UI AND API payload** via
  `toClientFiling` (relabeled "AM Best"; raw key never leaves the server).
- `build_ambest_df()` in `import_filings.py`: dedup â†’ window-filter â†’ premium-
  weighted rollup â†’ fail-loud mixed-source guard. Source-aware `verify()` keeps
...
- **Design + replacement path: `docs/AMBEST_INTERIM.md`.** Replace when scraped:
  `DELETE â€¦ WHERE source='ambest_sourced'` â†’ import scrape â†’ re-rollup.
- Cross-cutting state (WAF/quiet period/scraper) lives in the scraper repo's
  ...
  the June-21 cold-capacity read decides whether to upgrade IL/OH/VA interimâ†’real.

## Latest iteration â€” GA+NM data expansion + 13 brands + rate-neutral suppression (2026-06-16)

Big data + scope expansion of the rate-filing product (NOT the compliance
briefing â€” that's the prior iteration, still intact). The source xlsx was
...
### ðŸ”’ New verification baseline (REPLACES 468/314)

- **1,616 raw â†’ 998 rolled â†’ 293 active-window** (12mo from data as-of
  **2026-06-11**; `rate_change`/`rate_change_pending`; eff â‰¥ 2025-06-11).
- **13 brands, 10 states** (was 8/8). Date range **2024â€“2026**.
...
### âš ï¸ Intentional-but-odd states â€” DO NOT "fix" these as bugs
...
2. **GA + NM render compliance as COMING-SOON even though they have rate data.**
   ...
   briefing gate is `sectionsForState()` (WA/ID/UT only). Rate-data coverage â‰ 
   compliance coverage â€” this is the designed separation, not a missing wire.
...
3. **Rate-neutral (0.0%) filings are hidden from My Carriers + Overview only.**
   ...
   Change Type: Neutral" â€” verified against PDF `SFMA-134315020`: indicated
   ...
   **rate-moving** filingsâ€¦". A carrier whose filings are ALL neutral shows the
   normal no-data empty state (NOT coverage-gap â€” it IS covered).
...
4. **Positioning is COVERAGE-AWARE.** A brand counts as a competitor in a cell
   ...
   every non-GA cell (insufficient jumped 29â†’79). Fixed â†’ captive SF all-8 totals
...
5. **Coverage-gap empty state is data-driven + self-healing.** A selectable brand
   ...
   rows incl. 0.0% â€” that's why neutral-only â‰  coverage-gap).
...
- **Two collision guards (load-bearing â€” a wrong attribution silently corrupts
  Prospect/Defend):** Safeco rule matches the FULL "general insurance company of
  america" so "Nationwide General Insurance Company" â†’ Nationwide; USAA is
  checked before any generic match so "USAA General Indemnity" â†’ USAA.

### states.ts / coverage

- **GA + NM `data_coverage:true`.** Their `validated` flags' VALUES come from the
  AM Best cross-check logs â€” GA `{auto:true,home:true}` (PPA 92.8% / HO 94.3%),
  NM `{auto:true,home:true}` (PPA 97.3% / HO 88.2%) â€” but the ROWS themselves are
  ...
  state/line CROSS-CHECK flag, NOT a "this data is AM Best-sourced" marker â€” no
  app row is AM Best data; the four-tier `external_validation` gradient isn't even
  imported. `constants.ts` `COVERED_STATES` â†’ 10. Methodology text updated
  (2024â€“2026; 13 brands; "{50-n} states not covered"; newer/thinner-coverage note).
...
`coverage` + `neutralHidden`. Monorepo `cd86a5c` â†’ deploy `d3470cb`.

## Earlier iteration â€” Compliance briefing goes MULTI-STATE: +ID +UT (2026-06-16)

The compliance office briefing expanded beyond WA to **Idaho and Utah** via a
**per-state section model** â€” and in doing so PROVED the multi-state template
...
- **Per-state section model** (`briefing.ts`): each built state declares its own
  ...
  **unchanged** so WA renders exactly as before. `FEDERAL_DEFAULT_SECTIONS` (ID,
  UT) is 5 â€” wage, salary, leave, at-will, business tax â€” **no WA Cares**.
  `SECTIONS_BY_STATE` maps stateâ†’list; `sectionsForState()` /
  ...
  its own list â€” that's the whole extension point.
- **Honesty bar preserved on the new states.** ID/UT salary sections defer to the
  **federal FLSA** (no invented threshold figure); **UT at-will is intentionally
  left COMING-SOON** (no clean official Utah .gov source â€” grounding it weakly was
...
- **Coming-soon sections no longer assert size applicability.** The generic
  ...
  sections** â€” a coming-soon section hadn't grounded that claim. (UT at-will, AZ
  blocks no longer show it.)
- **Not-yet-built states** (AZ/CO/MT/NV/OR + any non-covered employee state) still
  render the whole-state coming-soon block, surfaced not dropped â€” multi-state mix
...
being briefing-ready (was the stale "only WA" world â€” no logic regression); new
`e2e_compliance` cases (ID-only, UT-only coming-soon at-will + no size note,
WA+ID+AZ mix). All green: tsc Â· 7 verify Â· e2e_compliance + e2e_nav Â· build 12/12.
Monorepo `a5c11a2` â†’ deploy `50ef430`.

## Earlier checkpoint â€” 2026-06-02 (office summary + accordion)

- Monorepo `b3a94ac feat(compliance): office summary + accordion briefing`.
- **Deployed (agent-intel/master): `71d448c`** â€” in sync with the monorepo
  subtree at that checkpoint. All suites green (6 verify + 13 e2e).

## Latest iteration â€” Compliance office summary + accordion (2026-06-02, deployed)
...
- **Profile inputs:** `pay_type` (Hourly/Salary/Both) and `remote_count` (whole
  ...
  drives a prompt (banner on `/setup`, upgrade card on `/compliance`) â€” never
...
- **Office summary** (`src/components/OfficeSummary.tsx`, above the disclaimer
  ...
  determination â€” blocklist-tested); and the **load-bearing out-of-state remote
...
- **Relevance links = primary nav into the briefing.** Expansion state lifted to
  ...
  (rAF â†’ `scrollIntoView`, honoring `scroll-mt-[88px]` so it lands below the sticky
  band) â€” never a collapsed empty row. No-dead-link rule intact:
  `briefingSectionAnchorId` only resolves when the target renders (no-WA profile â†’
  zero links; remote pointer has no section â†’ plain text).

Structure/presentation only; no change to grounded summary content or
determination-language honesty. Monorepo `b3a94ac` â†’ deploy `71d448c`.

## Earlier iteration â€” Overview: carrier-activity recent-filings card (2026-06-02, deployed)
...
filings. Selection order: (1) **coverage floor** â€” most-recent filing from each
licensed state the carrier filed in (every filed state guaranteed â‰¥1 row);
(2) **fill** â€” next-most-recent by recency only, capped at `CARRIER_ACTIVITY_EXTRA
...
from your carrier in: {states}", suppressed when the list is entirely empty â€”
the empty message covers that). Each row: carrier/line/sub-type, signed change,
state, effective date (with year), abbreviated policyholders â€” reusing
`formatEffectiveDate`/`formatPolicyholders`/`formatRateImpact`/`rateImpactColor`
+ `resolveSubtype` label cleanup. Added a "See all â†’" link to `/my-carriers`.
**No new query path** â€” same own-carrier set `/my-carriers` renders, a different
...
`cde0c95` â†’ deploy `cc7bf2a`.

## Earlier iteration â€” Methodology: Positioning section (2026-06-02, deployed)
...
"section-positioning"`. Monorepo `48cb173` â†’ deploy `6e261ea`.
...
```
1â€“13  v1 build order + polish                       done (deployed)
+  Post-v1 iterations (all shipped & deployed):
    ...
    - Feature 9: Compliance office briefing (/compliance)   â† newest
```

## Feature 9 â€” Compliance office briefing (complete, deployed)
...
- **Grounded figures shown** (decision B, reversed from the earlier qualitative
  decision A): min wage $17.13 (2026, up from $16.66); overtime 1.5Ã—; WA Cares
  ...
  pages â€” no number invented).
- **Salary/exempt threshold**: shows the formula for transparency
  (`2.25 Ã— $17.13 Ã— 40 = $1,541.70/week`, keeps "tiers currently match in 2026"),
  a **derived annual â‰ˆ$80,168/yr** computed in the UI as weekly Ã— 52 (ties by
...
- **Source links** show a distinguishing path segment (e.g.
  `dor.wa.gov/â€¦/business-occupation-tax-classifications`), not bare domain.
- **WA-complete.** All other employee states (covered-but-unmapped AND
  non-covered) render the honest **coming-soon** block â€” surfaced, not dropped.
  State expansion is a separate effort.

## â° Standing maintenance obligation â€” regenerate compliance figures yearly
...
regenerated at least once a year** â€” most WA figures reset on **January 1**
...
1. `ANTHROPIC_API_KEY=â€¦ npx tsx scripts/generate_compliance.ts --only WA/wage_hour,WA/salary_threshold,WA/leave,WA/wa_cares,WA/business_tax`
2. Verify the new numbers against the official source pages.
3. Commit + subtree-push â†’ redeploy.

The UI shows a per-section "as of {last_checked}" date so staleness is visible,
but that does not remove the obligation to refresh. (Spec: Feature 9 â†’
...
## Deploy workflow (two-repo topology â€” read before shipping)
...
2. `git subtree push --prefix=agent_intelligence agent-intel master`
   (from the monorepo root; remote `agent-intel` â†’ the standalone repo).
...
## Test suites â€” all green at acd848c (5 verify + 13 e2e)

verify_queries Â· verify_profile Â· verify_overview Â· verify_positioning Â· verify_subtype
e2e: setup Â· prospect Â· defend Â· my-carriers Â· overview Â· compliance Â· nav Â·
methodology Â· filters Â· empty_states Â· skeleton Â· mobile Â· positioning
...
- verify_*.ts are node-only (no browser). **verify_positioning is the Feature 7
  answer-key gate** â€” captive State Farm all-8 must produce 10 anchored /
  6 unanchored / 41 comparable / 24 higher-confidence / 17 thin / 29
  insufficient; independent {SF,Travelers,Progressive} â†’ 69 / 34. If these
...
## Item 6 â€” badge color audit (2026-05-29): CLEAN, no code change
...
- **`amber` is the correct single token for the spec's "Yellow" AND
  ...
  families â€” there is no separate orange/yellow token â€” so both
...
spec â†’ UI / design system â†’ Coverage warning) has no implementation in
...
operation â€” the banner would only fire on hand-edited `localStorage`.
...
1. **Tap targets at 28â€“30px: accepted.** The filter chips (30px) and
...
3. **Tailwind cache gotcha â†’ CLAUDE.md.** Added (this commit) under a
   new "## Gotchas" section so the next bulk-className edit doesn't
   hit a stale `.next` cache for the third time.

## Outstanding verification â€” RESOLVED 2026-05-29

**Info-dot circular `i` glyph visibility on mobile â€” VERIFIED.**
Confirmed at 375Ã—800 on `/prospect` (independent AZ+NV profile).
...
- Box 14Ã—14, `border-radius: 9999px` (circular), background
  `rgb(241,239,232)` (`bg-soft`, filled â€” not transparent),
...
- **Visual confirmation:** the clipped PNG shows the circular gray dot
  with an italic serif "i" â€” the shape renders, not just the text.
...
1. ~~Info-dot mobile check~~ â€” DONE (verified 2026-05-29, see above).
2. ~~Item 6: badge color audit~~ â€” DONE (2026-05-29, clean; see the
   "Item 6 â€” badge color audit" section above. No code change.)
3. **Step 13: deploy to Vercel** â€” IN PROGRESS. Deploy-prep done; the
...
- **File-tracing fix applied** (commit below): empty `next.config.mjs`
  ...
  reads of `data/` would NOT be bundled into the Vercel lambdas â†’
  guaranteed 500s, invisible at build. Added
  `experimental.outputFileTracingIncludes` (experimental-scoped for
  14.2.x; top-level is Next 15) mapping `/api/filings` â†’
  filings.db + last_updated.txt, `/methodology` â†’ last_updated.txt.
  **Verified from `.nft.json` trace manifests** that both files are
  traced and the relative entries resolve to the real files â€” proven,
...
`git subtree split --prefix=agent_intelligence` â€” full 26-commit history
...
- Remote `agent-intel` â†’ the standalone repo.
...
- Vercel project: connect **`AceVon3/agent_intelligence`**. Because the
  ...
  `agent_intelligence` â€” that note applied to the old monorepo plan).
...
- WA `wage_hour` and WA `leave` generated summaries were independently
  verified against the fetched source pages â€” every factual claim
  traced to source content, no fabrications.
- Fetch-fail fallback test passed (typed URL â†’ entry absent from
...
All 12 E2E suites pass at the most-recent commit:
  setup Â· prospect Â· defend Â· my-carriers Â· overview Â· compliance Â·
  nav Â· methodology Â· filters Â· empty_states Â· skeleton Â· mobile
